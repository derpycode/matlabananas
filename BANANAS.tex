...
\publisher{University of California, Riverside / Dept. of Earth Sciences 2018/9} % Publisher
...
Throughout ... but be aware, because it is probably not implemented particularly consistently: the following formatting in the text is used to distinguish the specific context of the word:
...
\item Math is hi-lighted in a slightly different font (and in \textcolor{blue}{blue}), e.g:
...
\section{Sidenotes and other distractions from the main text}

\sidenote{I am a Sidenote!} sort of things will appear in the text -- sidenotes\sidenote{I am also a Sidenote!}. There will be some corresponding text or comment in the margin. Most are helpful and offer additional guidance or suggestions and you should read.\sidenote{Some are trivial and worthless, but you wont know which is which until you have read them ...} In fact, the format of the book gives over substantial space for Sidenotes, explanation boxes, and figures, so be prepared that important information may appear in the margins.
...
for you to command it to go do something (stupid?). If one does not appear at the bottom of whatever is in the \textit{Command Window} is means that \textbf{MATLAB} is busy doing something extremely important. Or perhaps, \textbf{MATLAB} may have completely died. Either way, it will not accept any new/further commands until it is done calculating/dying.}.
...
\item \textbf{Integer} -- An integer number is a counting number, i.e. \texttt{1, 2, 3, ...} and including zero and negative integers.
 \textbf{MATLAB} has different representations for integer numbers, depending on how large a number you need to represent (and how much memory it will need to allocated to storing it). This is something of a throw-back to the days when computers only had 1/10000000\(^{th}\) of the memory of your iPhone and were slower than half a lemon nailed to the floor.\item \textbf{Real (floating point)}\sidenote{The distinction (sort of) is that \textit{floating point} is a specific representation of a \textbf{real} number.} -- A real number can have a non-integer component, e.g. 1.5 or 6.022140857 \(\times\) 10\(^{23}\). Real numbers also come in different precisions in \textbf{MATLAB} (also to do with memory allocation and speed), determining not just the number of decimal places that can be represented, but also the maximum size.
\item \textbf{String (character)} -- One or more characters, but now allowing spaces (unlike in the case of naming variables).
\item \textbf{Logical} -- the variable can be \texttt{true} or \texttt{false}\sidenote{As opposed to a Trump variable, that can have many different  alternative states of 'true', although generally, a Trump 'true' is in fact 'false'. An entire new branch of mathematics and logical deduction has been created just to process al this. } -- we'll come to quite what this means later.
...
The next thing to learn is ... to ideally, not attempt to mix up (combine) variables of different types. \textbf{MATLAB} is very forgiving when it comes to combining an \textit{integer} and a \textit{real} number in the same calculation, but in some other programming languages, this should be avoided.
However, even in \textbf{MATLAB}, \textit{strings} and \textit{reals} (or \textit{integers}) are very different things.\sidenote[][-1.25in]{Again -- in the \textbf{Excel} example, \textbf{Excel} will not let you add a \textbf{Number} and a \textbf{Text} value together, for instance. (Try it! You should see \textsf{\#VALUE!} reported.)} When necessary, different variable types can be converted between (see \textbf{Variable Type Conversion} Box).
...
A = 'banana'
\end{docspec}
\noindent has now replaced the real number \texttt{10} with the character string \textbf{banana} in variable \texttt{A}. This is reflected in the updated variable list details given in the \textsf{Workspace} window (and a \textit{Class} now listed as \texttt{char}).\sidenote{Equally in \textbf{Excel}, you can simply type over a pre-existing value to replace it. e.g. you could type \textsf{banana} over the contents of cell \textsf{A1} (that previous held the number\textsf{ 10).}}

Finally, it is possible to suppress output to the \textsf{Command Window} when making variable \textit{assignments} -- simply add a semi-colon (\docenvdef{;}) to the end of the \textit{assignment} statement\sidenote{Again -- your personal choice as to whether to include spaces or not between the \texttt{C}, \texttt{=}, \texttt{'banana'}, and \texttt{;} (Maybe try it both ways to convince yourself at least in this context, spaces do not matter.)}, i.e.
\begin{docspec}
C = 'banana';
\end{docspec}
\noindent now does not results in anything being echoed to the command line (but the \textsf{Workspace} is still updated to reflect this \textit{variable} assignment). 
%------------------------------------------------
...
\noindent and  test the relationship between 2 variables. \uline{Note in particular, that the \docenv{equality} symbol (that tests the equivalence between two variables) is represented by TWO \texttt{=} characters\texttt{ (==), }and remember that a single \texttt{=} character is the \docenv{assignment operator}.}
...
\textbf{MATLAB} provides numerous built-in \textit{functions}\sidenote{We will be constructing our own later, at which point it should become apparent that there is nothing particular special about them.}. These \textit{functions} have specific names assigned to them, so care needs to be take not to give a \textit{variable} the same name as a \textit{function} to avoid getting confused further down the road. Giving an exhaustive list (and brief description) is outside the scope of this document\sidenote{A full list of functions can be found in the \textbf{MATLAB} Help Documentation under \textit{functions}.}. Common \textit{functions} will be progressively introduced as this text  progress. Note that in addition to the on-line Help documentation, information on how to use a \textit{function} and example uses is provided by typing \texttt{help }and then the \textit{function} name (separated by a space) at the command line.

\textbf{MATLAB} also provides several built-in mathematical constants (saving having to define a variable with the appropriate number). This are simply variables that have been already defined and assigned values, but which  you cannot change (hence the term 'constant'). For instance,  the value of \(\pi\), is assigned to a built-in \textit{variable} with the name \docenvdef{pi}. You can access (display) its value  by typing its name at the command line:
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item Display the contents of the \textit{vector} in the command window by typing its name at the \textsf{command line}. Obviously, this will quickly become useless for very large \textit{vectors}\sidenote[][-0.0in]{Try creating a \textit{vector} from 1 to 100,000 and then displaying it ...}.
...
\noindent will turn the vector \texttt{B} into one (assigned to the \textit{variable} \texttt{D}) with he same orientation as \texttt{C}. \sidenote{Note ... \textbf{MATLAB} gives the syntax as \texttt{.'}, whereas I always only ever added the \texttt{'} bit ... which works ...} You can also use the \docenvdef{transpose} \textit{function}.

You can also re-order the values in a \textit{vector} (hence addressing the restriction in using the \textit{colon operator} to create a \textit{vector} that the values must be monotonically increasing rather than decreasing). Depending on the orientation of the \textit{vector}, you can use either the \docenvdef{flipud} (for column \textit{vectors}), or \docenvdef{fliplr} (for row \textit{vectors}) \textit{functions} to re-order the elements.
...
The command \docenvdef{figure} creates a \textsf{figure window}, which is where \textbf{MATLAB} displays its graphical output ... but on its own, without anything in it ... useless. So, lets put something in it, with the simplest possible graphical way of displaying data called \docenvdef{plot}. 
...
With any new \textbf{MATLAB} command (\textit{function}), get into the habit of looking up the \texttt{help} text  (also refer to alternative/simplified help provided in this text). The key information that will get you started appears at the very top of the text that \texttt{help} returns on \texttt{plot}:
...
\newthought{Question:} What is the sum of the 2nd row of \texttt{C}?
 For a \textit{matrix} (rather than a \textit{vector}) as input, \docenvdef{sum}  returns the individual sums of each column, and so on its own;
\begin{docspec}
>> C\\
C =\\
\ \ \ \ 255   119   187   102\\
...
This is where the \docenvdef{transpose} function (\texttt{'}) comes in handy (see earlier). In this case, it flips a (2D) \textit{matrix} around its leading diagonal (columns become rows, and rows, columns)\sidenote[][-1.0in]{This is almost true. Technically the function you want is \texttt{.'}, as \texttt{'} will change the sign of any imaginary components. For real numbers, they are the same.}
...
\marginnote[-0.0in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
{The command \docenvdef{addpath} will add a search path to the MATLAB workspace. e.g. \\\texttt{addpath DIRECTORY\_PATH} where \texttt{DIRECTORY\_PATH} is a \textit{string} (characters in between inverted commas) or name of a variable containing a string.}
\end{mdframed}}
...
... strange ... things are happening (as per Figure \ref{fig:ch1-plot-icecore_default}). In fact, \textbf{MATLAB} is doing what \textbf{Excel} would  in a \textsf{Line Chart} with 2 columns of data selected -- rather than plot y (2nd column) vs. x (1st column), the values of both columns are plotted against row number.\sidenote[][0.25in]{And this is why you should remember to use the \textsf{Scatter} (or \textsf{(X,Y)}) \textsf{Chart} in \textbf{Excel} for plotting (x,y) data.}

Instead, if you remember, the format of the \textbf{MATLAB} \texttt{plot} function is:

\begin{docspec}
plot(X,Y) plots vector Y versus vector X
\end{docspec}

\noindent So you need to specify each column of the data (i.e. each \textit{vector}) separately and explicitly. If you recall how to specify an entire row or column of an array, you should see that you need to type:

\begin{docspec}
>> plot(etheridge\_etal\_1996(:,1),etheridge\_etal\_1996(:,2));
\end{docspec}

If this is not obvious ... break it down and create sperate explicit x and y data vector variables, e.g.
...
\noindent For another example: download the historical global temperature anomaly dataset: \texttt{temperature\_globalanom.txt}\sidenote{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. The columns are: (1) year , (2) annual mean ocean+land surface temperature. Plot the annual mean for the full range of years, as per Figure \ref{fig:ch1-plot-temperature1}. \sidenote{plus labels, title, etc etc}
...
\sidenote{Remember -- you can increase the value of every element in an array, by simply adding that number, e.g. if \texttt{A} is you array of data, and \texttt{B} is a \textit{scalar} (the value you want to increase all array values by);
\begin{docspecmargin}
>> C = A + B;
\end{docspecmargin}
...
Create a new data array in \textbf{MATLAB}, with year as the first column (year, as per the original data) and the 2nd  column as annual mean  temperature in units of \degree F.\sidenote{Remember -- \textbf{MATLAB} will happily do the same to all values in an array  in an equation, as for a single scalar value in the same equation. Start by creating a new array with a single column, copied form the 1st column of the original array.} If it helps -- play  the data conversion game in \textbf{Excel} first (e.g. creating new columns in a spreadsheet to firstly hold  absolute temperatures rather than anomalies, and then temperatures in \degree F rather than \degree C). Also if it helps -- create a new array with the modified temperature units data in (rather than replacing the 2nd column of data in the original array). You can also do the conversion in 2 stages -- multiplying the (absolute) temperature (\degree C) by \(1.8\) first (perhaps creating a new array to hold this in), then adding \(32\).
...
\noindent For some practice in a little further data manipulation and graphing: download the historical temperature dataset for Riverside: \texttt{temperature\_riverside.txt}\sidenote{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. If you view the data file in a text editor or import into \textbf{Excel}, you can read the column headers and find out what the different columns of data are (I am not telling you!). Note that the (1st) line of the file containing the column header labels starts with a \% symbol (telling \textbf{MATLAB} to ignore this line and not attempt to read in the 'data' on it).
...
\marginnote[-0.0in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
 \% 
\\This is a special symbol that when \textbf{MATLAB} sees it, it ignore the entire line. This is known as a comment symbol (of identifier) and allows you to have lines of comments in amongst the lines of code.
...
First create a plot (with appropriate labels) of this data.
...
\noindent from which we wish to remove the 2nd row. In MATLAB we would create the vector by:
...
Play this 'game' -- deleting as many row as you think result in biased means (because of missing monthly data)\sidenote[][-0.5in]{(being aware that as you delete rows, the numbering of the subsequent rows changes as the array size shrinks ...)}, with the Riverside temperature data, and re-plot the results.
...
The axis and title labels, by default, can be difficult to read when the graphics are saved and then imported into a document/paper. You can change the size of text as you create axis captions and figure titles etc., by specifying the value of an additional (text size) parameter in the function. For example, to increase the size of the xaxis label to a 14pt font:

\begin{docspec}
>> xlabel('Year','FontSize',14);
\end{docspec}

Here -- after the you have passed the string you wish ot appear to the \textbf{MATLAB} \texttt{xlabel} \textit{function} (\texttt{'Year'}), there is a pair of additional parameters:
...
Other property parameters\sidenote{Again -- refer to \textbf{MATLAB} help.} that might be useful (to change), are (with example changes):
...
Well, it is a good job that you remember how to re-orientate arrays, right?\sidenote[][0.0in]{You don't? See earlier in the Chapter ...} If you guess right first time (three different basic transformations of a matrix were described), you get Figure \ref{fig:ch1-plot-pcolor2}.
...
\marginnote[1.25in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{pcolor}
