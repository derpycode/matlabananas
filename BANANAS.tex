...
\end{docspec}
\noindent but ... with 2 important differences. Firstly, rather than specifying the 1st, then 2nd, etc layer of the 3D array, here we are specifying the layer with an index equal to  the contents (or value) of \texttt{month}, which, if you remember, counts up from 1 to 12 in the \textit{loop}. Secondly, rather than specifying the filename explicitly in the load command, we are passing the string contained in the variable \texttt{filename}. Hopefully on the previous line of code within the loop, you have created the string value of \texttt{filename} ...

Assign the  2D data array that is loaded in, to the \texttt{temp} array variable, specifying the layer number equal to the month number, i.e.

\begin{docspec}
temp(:,:,month) = load(filename);
\end{docspec}

Run this and take a look at the \textsf{Workspace window} -- note that you have an array (\texttt{temp}) that has size 94\(\times\)192\(\times\)12. If temp is 94\(\times\)192\(\times\)1 then go back a page or so and go through the bit about loading data into a 3D array. You want to avoid over-writing the information that is already there, so the line;
...
\noindent Finally ... look up \textbf{MATLAB} \texttt{help} on \docenvdef{getframe}. Then go back to your global temperature loading/plotting script and add the following line\sidenote{Where to put the line? See the \texttt{Example} given in the \texttt{help} on this function. It is exactly what you are doing here.}:
\begin{docspec}
M(month)=getframe;
\end{docspec}
...
Finally -- there are \textbf{MATLAB} commands to turn your \textbf{MATLAB} format movie into a format you can use elsewhere. Previously ... there was a command was \docenv{movie2avi} (see Box). But it has been ... retired (curse you, Mathworks). The new/replacement command is \docenvdef{VideoWriter,} which differs mostly in that the animation is now created within the program and the  \textsf{.avi} format animation has its frames added (within the loop) as the graphics are created (rather than converting the frames afterwards as in \docenv{movie2avi}).

To use the \textbf{MATLAB} \docenv{VideoWriter} \textit{function}, adjust your code as follows (and as per the \textbf{MATLAB} \textsf{help} on \docenv{VideoWriter}):
