...
\ \ \    \textcolor{blue}{if} (~isempty(found\_nans));\\
\ \ \ \ \ \        \textcolor[rgb]{0,0.501961,0}{\% remove row}\\
\ \ \ \ \ \        data(n,:) = [];\\
\ \ \ \ \ \        \textcolor[rgb]{0,0.501961,0}{\% remember ot update total number of rows!!!}\\
\ \ \ \ \ \        n\_max = n\_max - 1;\\
...
\noindent takes the values told at x-values \texttt{xold}, and interpolates onto the x-values \texttt{xnew} (and assigns the new y-values to \texttt{ynew}). For instance, if we have the following crudely spaced data\sidenote{In MATLAB notation:
\begin{docspecmargin}
...
\noindent and we wanted to create an interpolated dataset from \(0.0\) to \(30.0\) (x-axis values) in steps of \(1.0\), we would first create the new x-axis vector that the data will be interpolated on to:
...
\noindent and obtain the interpolated data as shown in Figure \ref{fig:ch3-interp1}.
...
\noindent We can write this as an (normalised) scale vector \(x\) together 3 color vectors (for red, green, and blue):
...
If we want to create a scale of 11 (from \(0.0\) to \(1.0\) in steps of \(0.1\)) different colors, we can create a new \(x\) vector to interpolate on to:
...
\noindent etc., or \textbf{MATLAB} allows us to combine them:
...
\begin{docspec}
mapnew = interp1(xold,colold,xnew,'spline');
\end{docspec}

If you now set the new \textit{colormap} (\texttt{colormap(mapnew);}) and re-plot the global topography -- Figure \ref{fig:ch3-etopo6}.
