...
\end{docspec} 
where we are now specifying random values (in the range 0-1) for each of the three red, green, blue
color intensity components (see earlier).
...
So yes, this is a built-in \textit{function} \texttt{max} in \textbf{MATLAB}, but suspend disbelief for a moment ... and pretend that there is not one. What if we wanted to create one, i.e. a \textit{function} that is passed a vector of numbers, and returns the maximum value on that vector?
...
So at the top of the \textit{function} (but below the comment lines), you could create a \textit{variable}, set equal to the number of elements in the \textit{vector} that you are going to have to process:
\begin{docspecbold}
nmax = length(v\_in);
\end{docspecbold}
...
\newthought{If you are passably happy with that --} write a (new) \textit{function} that finds the \textit{minimum} element in a vector input.\sidenote[][-0.0in]{This is in fact employed a little later, where it is referred to as \texttt{minxx}.}
...
\newthought{Then ... as a 3rd function --} return a second variable from the function -- equal to the number of elements that are equal to the maximum.\sidenote[][-0.0in]{If we were cheating, and we \uline{are not} ... then one could use the built-in \textbf{MATLAB} functions as so:
...
You might try exploring what youn are doing to 'do' with 2 outputs ... e.g.. if you (temporarily) added the following lines to your \textit{function}:
\begin{docspec}
s\_out1=1.1;\\
s\_out2=2.2;
\end{docspec}
