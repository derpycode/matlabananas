...
>> help sort
\end{docspec}
Actually ... not quite. Reading the help text carefully (and you can always try it out and see what exactly it does if you are not sure), \docenv{sort} will sort all columns independently of each other, whereas we want the first column sorted and the remaining columns linked to this order. 

Under \texttt{see also}, \textbf{MATLAB}\ lists \docenvdef{sortrows} as a possibility. The \textsf{help} text on this looks a little more promising. It is still slightly opaque, so the best thing to do is to try it (and view the results)! 

This looks rather better. The result of \texttt{plot}-ting this is Figure \ref{fig:plot-CO2basic2}. (This is a good illustration of a guess of a function that was not quite what was needed, but following up on the \texttt{help} suggestions leads to a more appropriate function.) At least now the curve is reminiscent of past changes in global temperature and the geological Wilson cycle, with high values in the Cretaceous and Jurassic and then lower again in the Carboniferous (roughly matching the progression of ice and hot house (and then back to recent ice ages) climates).
...
Shortly, you will see an alternative plotting function that does not require the data to be sorted into any sort of order. Note that you can also use \docenv{plot}, and omit the line segments by specifying only a symbol, e.g.
...
\vspace{1mm}
\noindent \texttt{temperature\_globalanom.txt}\sidenote[][-0.0in]{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. \vspace{1mm}

The columns are: (1) year , (2) annual mean ocean+land surface temperature \uline{anomaly} (i.e. temperature change relate to some reference value, where here is the observed 1901-2000 mean). Plot the annual mean anomaly for the full range of years, as per Figure \ref{fig:ch1-plot-temperature1}. (plus labels, title, etc etc).
...
\sidenote[][-0.5in]{Remember -- you can increase the value of every element in an array, by simply adding that number, e.g. if \texttt{A} is you array of data, and \texttt{B} is a \textit{scalar} (the value you want to increase all array values by);
\begin{docspecmargin}
>> C = A + B;
\end{docspecmargin}
...
For this, you will need to take your data (which is the 2nd column of the array), e.g. data(:,2), multiple by 1.8 as per the equation (\texttt{1.8*data(:,2)}) and then add 32 (\texttt{1.8*data(:,2) + 32})\footnote{If you have any doubts as to the order in which the operators are applied, add a set of parentheses, e.g. \\ \texttt{(1.8*data(:,2)) + 32}}. And ... assign the results of this to a new vector or array. Or you can replace the original column (if you are feeling confident), e.g.

\begin{docspec}
>> data(:,2) = 1.8*data(:,2) + 32;
\end{docspec}
 
\noindent The aim is to obtain a data array in \textbf{MATLAB}, with year as the first column (year, as per the original data) and the 2nd  column as annual mean  temperature in units of \degree F.\sidenote[][-0.0in]{Remember -- \textbf{MATLAB} will happily do the same to all values in an array  in an equation, as for a single scalar value in the same equation. Start by creating a new array with a single column, copied form the 1st column of the original array.} 
...
\vspace{1mm}
\noindent \texttt{temperature\_riverside.txt}\sidenote[][-0.0in]{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. \vspace{1mm}
...
First create a plot (with appropriate labels) of this data. You want the annual mean temperature -- the last column, plotted vs. year (the first column).
