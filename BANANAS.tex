...
Here we'll devise a basic version requiring 2 (mostly) human players, but note the possibility of developing an AI computer player. We'll also initially not make use of the \textbf{MATLAB} GUI, so keep the code as simple as possible, but note that as a further possibility for development.
...
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item We need a \textsf{Figure window} (no really)! No axes/axes labels showing would be nice.
...
This is my \textit{function} for drawing the grid (which will be called from the main program \textit{script} file):
\begin{docspecbold}
...
Somewhere after \texttt{hold on} (refer to Figure \ref{fig:ch6-gamex}), in your main program (script \textsf{m-file}), add the line\sidenote{Remember you are not passing any parameters to this function, nor is it returning anything back to you}:
\begin{docspec}
draw\_grid();
\end{docspec}
...
To try this out -- to the code in \texttt{game.m} (and somewhere after  \texttt{hold on}), you could add:
\begin{docspecbold}
[x,y,button] = ginput(1);
\end{docspecbold}
which when you click a mouse button, sets the \textit{variables} \texttt{x} and \texttt{y} equal to the (x,y) location of the mouse button click, and the \textit{variable} \texttt{button} to the ID corresponding to which mouse button it was.
...
Create a similar shape for \texttt{draw\_circle} ... maybe picking a different color, or a different shape. And then test this \textit{function} (at the command line) also.

\vspace{1mm}
\noindent\rule{4cm}{0.5pt}
\vspace{2mm}

\noindent We should be in good shape at this point -- you have a main program that draws the game grid, and finds the \((x,y)\) coordinates of a mouse button click. And you have a pair of functions to draw a shape centered on \((x,y)\), although so far you have not joined this up and have only tried passing whatever value you fancy into the square/circle plotting \textit{functions}.

You could test the code further by allowing multiple mouse-clicks and shape drawing -- in the main program, after you have drawn the game grid (and after the \texttt{hold on}) and replacing the temporary line of code you added earlier, i.e.
\begin{docspec}
[x,y,button] = ginput(1);
\end{docspec}
create a \textit{while} loop that initially, is endless, and contains the \texttt{ginput} function line:
\begin{docspecbold}
game\_on = true;\\
...
\ \ \        \textcolor{blue}{end}
\end{docspec}
where \texttt{xi} stands for a variable containing the index (or 'integer') \textit{x}-direction, i.e. which one of the 1st, 2nd, or 3rd, column it is. You could write something similar for the \textit{y}-direction.

Now, when we call \texttt{draw\_cross} (or \texttt{draw\_circle}), obtain the (\textit{x,y}) coordinates of the mouse click, convert to integer values representing the row or column number (index) as per the \texttt{\textcolor{blue}{if}} statement above ...  if we pass the value of \texttt{xi} (and \texttt{yi}), minus 0.5 -- representing the mid-point value of the cell in which the mouse was clicked. i.e. 
\begin{docspec}
...
To test this, and/or what you code should look like so far -- after the \texttt{hold on} statement in the initial code framework (the very first pie of code given to start building the \texttt{game.m} program) is:
\begin{docspecbold}
game\_on = true;\\
\textcolor{blue}{while} game\_on\\
\ \ \    [x,y,button] = ginput(1);\\
\ \ \  ...
\end{docspecbold}
then one set of code to identify the (integer) column (\texttt{xi}) number:
\begin{docspec}
\ \ \    \textcolor{blue}{if} (x < 1)\\
\ \ \ \ \ \             xi = 1;\\
\ \ \        \textcolor{blue}{elseif} (x < 2)\\
...
\ \ \        \textcolor{blue}{end}
\end{docspec}
... and then a further \texttt{\textcolor{blue}{if}} structure to determine which mouse button was clicked and draw a 'cross' or a 'circle' depending on which one it was:
\begin{docspec}
...
\end{itemize}
(so the elements of the array can take a value of 0, 1, or 2, as illustrated in Figure \ref{fig:ch6-grid2}). The array has been initialized to all zeros, and at this point the game crosses has already won.
...
Now ... in the loop, as the \textit{xi} and \textit{yi} indices are derived -- use them to assign a value to the \texttt{tokens} array.\sidenote[][0.5in]{It is a personal preference whether to simply remember that \textbf{MATLAB} indexes arrays differently to reading a normal \textit{(x,y)} location, or try and make the contents of the array, as viewed, look like the game grid.} You can then test  the value of the location that has just been chosen and from this decide whether the move is legal or not, e.g.:
\begin{docspec}
...
If you wanted to try out this code fragment, it would go in the
\begin{docspec}
\ \ \    \textcolor{blue}{if} (button == 1)\\
\ \ \ \ \ \  ...\\
\ \ \        \textcolor{blue}{end}
\end{docspec}
code bit under if \texttt{button == 1} is \texttt{true} (a cross)\footnote{and in place of the single line \texttt{draw\_cross(xi-0.5,yi-0.5);}}.
...
\ \ \ \ \ \         player1 = \(\sim\)player1;
\end{docspec}
All this goes still within the \texttt{\textcolor{blue}{if}} test of whether the move is legal or not, i.e.:
\begin{docspec}
\ \ \     \textcolor{blue}{if} (tokens(xi,yi) == 0),\\
\ \ \ \ \ \         ...\\
\ \ \     \textcolor{blue}{end}
...
\\This code would go just before the end of the \texttt{while} \textit{loop}.
\end{enumerate}
...
A slightly different alternative is to use \texttt{find}, and for each played, obtain the list of linear indices of the grid cells containing their symbol. You can test the figure example (and matrix above) by creating the corresponding matrix in \textbf{MATLAB}:
\begin{docspec}
A = [1 0 0; 1 0 0; 1 0 0];
\end{docspec}
...
winning = [1 2 3];
\end{docspec}
(for the first pattern),  we can  determine whether the elements of winning appear in  the linear indices of all of the players squares (extracted from the array by \texttt{find(A==1)}) by using the \textbf{MATLAB} function \docenv{ismember}:
\begin{docspec}
>> ismember(winning,find(A==1))\\
ans =\\
\ \ \      1     1     1
\end{docspec}
where the three \texttt{1}s indicate that each of the elements of \texttt{winning}, appear in the result of \texttt{find(A==1)\sidenote[][-0.0in]{The '\texttt{1}' because this is the notation for player 1.}}. Only if three \textit{trues} (\texttt{1}s) are returned, does the pattern completely match. So we can calculate  the sum of the result of \docenv{ismember} and whether this is  equal to \(3\), to indicate to us that the winning pattern exists.

Your job -- define the \(8\times 3\) array \texttt{winning}, containing the 8 different winning patterns (rows), in terms of a linear index (i.e. what \texttt{find} returns).\sidenote[][-0.0in]{The way to go about it is to create a single winning pattern, and text the code and that it works, then define the remaining 7.}\sidenote[][-0.0in]{Also -- write down on paper, the linear indices of the \(3\times 3\) array -- that will help, e.g.:
\vspace{1mm}
\\\(\begin{pmatrix}1 & 4 & 7 \\
2 & 5 & 8 \\
3 & 6 & 9 \\
...
For now -- prescribe the value of \(S\) -- for which the modern value is 1368 Wm\(^{-2}\) (\(S_{0}\)) as well as the value of surface albedo (\(\alpha=0.3\)  by default) -- somewhere near the start of the program (see Figure \ref{fig:ch8-schematic-scr1}). Then run it.
...
[Note that in this section, variables names are shortened and simplified as compared to before, with \texttt{S} replacing\texttt{ solar\_constant}, and \texttt{T} replacing \texttt{temp}. Feel free to retain the previous (or whatever you like) naming convention.]

\vspace{2mm}
\noindent Now to utilize your new function ('\texttt{fun\_1}' in the summary notation). Create a new blank script ('\texttt{scr\_2}') and define 2 parameters near the start -- one for the value of \(S\) and one for \(\alpha\), then further down the code, call your function (\texttt{fun\_1}), passing it these 2 parameters \uline{but remembering} that you need to assign the result of your function (\texttt{fun\_1}) to some \textit{variable}\sidenote{i.e.
\begin{docspecmargin}
T = fun\_1(1368.0,0.3);
\end{docspecmargin}assigns the result of your temperature calculation to the variable \texttt{T}.}. So far so boring, as this is in effect what you had been doing in 'playing' with the energy balance function previously. 
...
We'll start with a simple 1-D case, and consider just a change in the value of \(S\) (which is effectively what you have been doing as a function of time and using the solar constant function (\texttt{fun\_2}), anyway). To automatically generate a progression of different values of \(S\) and call the function, you are going to need to create a loop\sidenote{You are going to put the loop in the script (\# \texttt{scr\_2}), NOT the function (\# \texttt{fun\_1}).
...
\\Perhaps a little inconveniently, this does not pass exactly through the modern value (1368 Wm\(^{-2}\)), although when you plot as a continuous line (e.g. in \texttt{plot}) or otherwise interpolate the results, maybe this does not matter.  Remember, you could also interpolate the result later (e.g. on a new vector of solar constant values that include \texttt{1368}).
...
Or ... you could have made the loop go around in steps of \texttt{1} (and hence passing through a value of \texttt{1368}) for a total of 501 loop iterations(!)
...
\item [\textbf{save option \#1}] Create the necessary array(s) beforehand, e.g. using the \texttt{zeros} \textit{function}. For instance, to create a vector with 11 rows (and 1 column), suitable for saving the value of \(T\) calculated by each call to your EBM function (\textsf{fun\_1.m}), you could write:
\begin{docspec}
data\_T = zeros(11,1);
\end{docspec}
...
Pick one (i.e. a way of saving a pair of values each time around the loop) and code it up, or better, try all of them!
