...
A couple of common basic data manipulation/processes techniques follow, with some further plotting/visualization work tagged on.
...
Shortly, you will see an alternative plotting function that does not require the data to be sorted into any sort fo order. You can also use \docenv{plot}, and omit the line segments by specifying only a symbol, e.g.

\begin{docspec}
plot(x,y,'ro');
\end{docspec}

\noindent (here, plotting circles for the data points in red)

%------------------------------------------------

\subsection{Data scaling}

\noindent For an example practicing some basic data scaling: download the historical global temperature anomaly dataset:

\noindent \texttt{temperature\_globalanom.txt}\sidenote[][-0.75in]{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. 
\\The columns are: (1) year , (2) annual mean ocean+land surface temperature. Plot the annual mean for the full range of years, as per Figure \ref{fig:ch1-plot-temperature1}. (plus labels, title, etc etc).
...
\noindent For some practice in data filtering (row deletion) and a little further data manipulation and graphing: download the historical temperature dataset for Riverside:

\noindent \texttt{temperature\_riverside.txt}\sidenote[][-0.75in]{\href{https://www.ncdc.noaa.gov/cag/global/time-series/globe/land\_ocean/ytd/12/1880-2017/\#https://climate.nasa.gov/vital-signs/global-temperature/}{NOAA}}. 
...
So back to the temperature data -- to for example remove the row containing the year 2008 data\sidenote[][-1.0in]{You can also delete rows (and columns) if you open up the \textbf{MATLAB} \texttt{Array window} (double-click on the variable name in the \texttt{Workspace window}). And ... edit/replace values ...}, which is row 11, we write:
...
An illustration (and a far from perfect illustration) of a short \textit{function} (\textit{m-file}) exhibiting at least a few examples of good practice, is:
...
Finally, it is important to note that by default, any variables created within a \textit{function} are TOP SECRET, and by that, I mean that they are not accessible to the main \textbf{MATLAB} workspace and do not appear listed in the \textsf{Workspace window}. To see that this is a non-Trump \uline{true fact}, create the following \textit{function} (basically, the first example but split into 2 steps):
\begin{docspec}
...
How many bananas could you eat in a day? I bet it is less than ten. We'll let the computer ask and if the answer is 10 or more, you (the computer) shouts: 'liar!'.\sidenote{This example is even more stupid than the last one. But no more stupid than in any computer programming textbook and it will at least demonstrate a subtly different usage of \texttt{if ...}.}
...
Finally -- there are \textbf{MATLAB} commands to turn your \textbf{MATLAB} format movie into a format you can use elsewhere. Previously ... there was a command was \docenvdef{movie2avi} (see Box). But it has been ... retired (curse you, Mathworks). The new/replacement command is \docenvdef{VideoWriter,} which differs mostly in that the animation is now created within the program and the  \textsf{.avi} format animation has its frames added (within the loop) as the graphics are created (rather than converting the frames afterwards as in \docenv{movie2avi}). . To use this new \textbf{MATLAB} \textit{function}, adjust your code as follows and as per the \textbf{MATLAB} \texttt{help} on \docenv{VideoWriter}.
...
The code above also simplifies things by not bothering to create a 3D array, but rather over-writing a 2D array (\texttt{temp}) each time around the loop (having first created the animation frame).
