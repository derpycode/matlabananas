...
\publisher{University of California, Riverside / Dept. of Earth Sciences 2017/8} % Publisher
...
\item \textsf{Sans-serif font family typeface} -- indicates keyboard keys (e.g. \textsf{F5}), program menu items (e.g. \textsf{Save as ...}), program window names, and filenames (except where used in \textbf{MATLAB}).
...
\textbf{MATLAB} \texttt{help} is not always especially helpful. In the course text, for each \textit{function} that MATLAB provides a comprehensive help on, such as \docenv{help}, a simple summary version will be displayed in the right hand margin in a grey box. For example -- the box headed \texttt{\textcolor[rgb]{0.501961,0,0.25098}{FUNCTION}}.
...
\noindent When the given examples additionally illustrate how they are typed in plus the 'result' OR, requires you to type in the lines at the command line, the text is again highlighted text in a '\texttt{Typwriter}' font but in addition, the command line prompt (\textgreater\textgreater)\ is shown at the start of a line (you do not type in the prompt ...), e.g.
...
\noindent When you see a string or variable name in all CAPITAL LETTERS -- this is a 'placeholder' and is indicating that you should substitute in an appropriate string or variable name in its place, e.g. 

\begin{docspec}
load('FILENAME','-ascii');
\end{docspec}  

is indicating that you substitute the name of your actual file in place of \texttt{FILENAME}. Alternatively:

\begin{docspec}
plot(MYARRAY(:,1),MYARRAY(:,2));
\end{docspec}

would indicate that you should substitute your actual variable name (holding the data to plot in this example) in place of \texttt{MYARRAY}.
 
%------------------------------------------------

\section*{Code structure}

A visual guide to the structure of your programs is given by schematic figures in the page margin. For example, a generic \textit{script} (yellow box) is shown by Figure \ref{fig:ch0-script}, and a generic \textit{function} (green box) by Figure \ref{fig:ch0-function}.
...
The purpose of these cartoons is to help you when faced with a blank page and the question: 'Where do I start' or 'What do I write' appears prominently in your mind\sidenote{Also surrounded by flashing neon lights.}. It is to give you some sort of idea what bits might go where, and what general content is required in the file. The cartoons \uline{do not} (and are not intended) to show the exact details of the code content. Nor do they necessarily indicate all the different sections needed. Conversely, not all the sections illustrated may be strictly necessary and in come examples there may be nothing to 'initalize' and there may be no constants of local parameters to define the values of at the program start.

So please -- use the cartoons as a simple visual guide to the approximate structure of your program and \uline{do not over-interpret them}.
...
\noindent or in order versions of the software:
...
\noindent but in either case, with a vertical blinking line (cursor) following the double 'greater than' symbols\sidenote{Note that in nerd-speak the \texttt{>>} is called the command 'prompt' and is prompting you to type some input (Commands, swear words, etc.). See -- the computer is just sat there waiting
for you to command it to go do something (stupid?). If one does not appear at the bottom of whatever is in the \textit{Command Window} is means that MATLAB is busy doing something extremely important. Or perhaps, MATLAB may have completely died. Either way, it will not accept any new/further commands until it is done calculating/dying.}.
...
To convince yourself that nothing dreadfull will happen ... type ... anything. Actually 'anything' will do.
...
In the course text, for each \textit{function} that MATLAB provides a comprehensive help on, such as \docenv{help}, a simple summary version will be displayed in the right hand margin in a grey box.\sidenote{Refer to the section on 'How to use this Textbook'.}
...
A \textit{variable}\index{variable} is, in a sense, a pointer to a location in computer memory where a piece of information is stored\sidenote{In the bad old days, this pointer was the actual address in memory and might have looked something like \texttt{f04da105}.}. For instance -- open up a blank worksheet in \textbf{Excel}, and in the very top left hand cell, enter the number \textsf{10}. You can see visually, that Excel is referencing this location as column A, and row '1'. In fact, this location ('\textsf{A1}') is indicated in the \textsf{Name Box} to the left of the \textsf{Formula Bar}.

In \textbf{MATLAB}, a variable is associated a name to make things rather more easy and convenient. The name can be any sequence of characters you like in \textbf{MATLAB}, regardless of whether it is a real or fake word, as long as it does not contain numbers or special characters or spaces. So actually, you are only left with continuous sequences of letters (otherwise knows as 'words'). But you can also create a variable name based on 2 (or more) words, separated by an underscore (\texttt{\_}). Valid variable names  include:
...
BIG\\
big\sidenote{Note that MATLAB distinguishes between lower and UPPER case letters in a variable (i.e. \texttt{BIG} and \texttt{big} would represent two different and distinct variables). I would \uline{strongly} advise to stick to all lower case, or all upper case, to avoid possible future confusion. (or come up with a naming convention, of whatever sort (e.g. capital first letter), and stick to it.)}
\end{docspec}  

Variables are entirely useless unless they have some information assigned to them. In fact, you can type in any of the variable names above (at the command line) and MATLAB will deny it knows what you are talking about\sidenote[][0.25in]{Technically, MATLAB reports: \texttt{Undefined function or variable} which tells you it is neither a function name (more on this later), nor is defined as having any information associated with it.}.

So far so useless -- you need to \textit{assign} something to it. (When you first open an \textbf{Excel} spreadsheet and it is completely blank -- you can still reference cell A1, but there is nothing in it.) Which brings us to quite 'what' and 'how'. First of, you need to know that variables can have the following \textit{types} of things assigned to them:

\begin{itemize}
\setlength{\itemindent}{.2in}
\item \textbf{Integer} -- An integer number is a counting number, i.e. \texttt{1, 2, 3, ...} and including zero and negative integers.
 \textbf{MATLAB} has different representations for integer numbers, depending on how large a number you need to represent (and how much memory it will need to allocated to storing it). This is something of a throw-back to the days when computers only had 1/10000000\(^{th}\) of the memory of your iPhone and were slower than half a lemon.\item \textbf{Real (floating point)}\sidenote{The distinction (sort of) is that \textit{floating point} is a specific representation of a \textbf{real} number.} -- A real number can have a non-integer component, e.g. 1.5 or 6.022140857 \(\times\) 10\(^{23}\). Real numbers also come in different precisions in \textbf{MATLAB} (also to do with memory allocation and speed), determining not just the number of decimal places that can be represented, but also the maximum size.
\item \textbf{String (character)} -- One or more characters, but now allowing spaces (unlike in the case of naming variables).
\item \textbf{Logical} -- the variable can be \texttt{true} or \texttt{false}\sidenote{As opposed to a Trump variable, that can have many different completely alternative states of 'true'.} -- we'll come to quite what this means later.
\item \textbf{etc} -- No, not a real type, but to note that \textbf{MATLAB} defines and recognises a whole bunch of other types, including \textbf{Complex} (\textbf{MATLAB} can handle \textit{complex numbers}) and \textbf{Object} (we will also not worry about \textit{objects}, which can incorporate a combination of types. At least, not yet ...). The \textbf{MATLAB} documentation contains a full list (and/or go Internet Fishing).
\end{itemize}

To come back to Excel -- if you select \textsf{Format Cells} (right-mouse-button-click over cell A1), you get to chose from a long list of 'formats', including \textsf{Number} and \textsf{Text}, and which have a loose correspondence with \textit{types} in \textbf{MATLAB}.

The next thing to learn is to ideally, do not attempt to mix up (combine) variables of different types. \textbf{MATLAB} is very forgiving when it comes to combining an \textit{integer} and a \textit{real} number in the same calculation, but in other programming languages, this should be avoided.
However, even in \textbf{MATLAB}, \textit{strings} and \textit{reals} (or \textit{integers}) are very different things.\sidenote[][-1.25in]{Again -- in the Excel example, \textbf{Excel} will not let you add a \textbf{Number} and a \textbf{Text} value together, for instance. (Try it! You should see \textsf{\#VALUE!} reported.)} When necessary, different variable types can be converted between (see \textbf{Variable Type Conversion} Box).
...
The second and perhaps rather more important thing, is how to assign a value to a variable (and in fact, create the variable in the first place). Programming languages such as \textbf{FORTRAN} require you to define the variable beforehand and assign it a type.\sidenote{Partially true. An Alternative Fact of sorts.} \textbf{MATLAB} allows you to define and assign a value to a variable all at the same time, and it will kindly work out the correct type based on the value you assign to it. You assign a value using the assignment operator \docenvdef{=}\sidenote{This is NOT 'equals' in \textbf{MATLAB}. Or any sane programming language. We will see the \textit{equality operator} shortly. \docenv{=} assigns the value or variable on its right, to the variable on the left.}. For example:
\begin{docspec}
A = 10
\end{docspec}
...
\textbf{MATLAB} will also report in the \textit{Workspace} window, the name and value, \textit{type} (unhelpfully called \textit{Class}), etc of all your current variables (just one currently?). Actually, it is not all quite so simple. If you take a look at the \textit{Class} of the variable \texttt{A} in the display window -- it is  listed as \textit{double} (a \textit{real} number) rather than an \textit{integer}. So by default, if \textbf{MATLAB} does not know what you really want, it defines \texttt{A} as a double precision real number\sidenote{If you genuinely wanted an integer, there are ways to do this, such as using a type conversion function form \textit{real} to \textit{integer} (see above).}.

Pausing again -- if you want to remind yourself of the variables that you (or a program) have created -- you can refer to the \textit{Workspace} window.\sidenote{There is a command line command for listing current variables (\texttt{whos}), but lets not bother with it.} Also lister here is its value (and \textit{type} etc). Another way to access the value of a variable, is to simply type in its name at the command line:
\begin{docspec}
>> A
...
B = apple
\end{docspec}
\noindent and \textbf{MATLAB} is far from happy. As it turns out,  a sequence of characters  can also refer to a \textit{function}\sidenote{You will see \textit{functions} shortly. For now -- note that they are 'special' (reserved) words that perform some action and hence cannot also be used for a variable name.} in \textbf{MATLAB}, and this is what \textbf{MATLAB} looks for (i.e. a match to \texttt{apple} in the list or variable (and function) names). In other words, \textbf{MATLAB} does not know whether you intend \texttt{apple} to be a \textit{string} or a \textit{function}. It assumes function ... but cannot find one with that name. To delineate \texttt{apple} as a \textit{string}, you need to encase it in (single\sidenote{Double \texttt{""} quotation marks will not work.}) quotation marks:
\begin{docspec}
B = 'apple'
\end{docspec}

Just as \textbf{MATLAB} creates new variables on the fly, you can re-assigned values to an existing variable, even if this means changing the \textit{type}, e.g.
\begin{docspec}
A = 'banana'
\end{docspec}
\noindent has now replaced the real number \texttt{10} with the character string \textbf{banana} in variable \texttt{A}. This is reflected in the updated variable list details given in the \textit{Workspace} window (and a \textit{Class} now listed as \texttt{char}).\sidenote{Equally in \textbf{Excel}, you can simply type over a pre-existing value to replace it. e.g. you could type \textsf{banana} over the contents of cell \textsf{A1} (that previous held the number\textsf{ 10).}}

Finally, it is possible to suppress output to the \textit{Command Window} when making variable \textit{assignments} -- simply add a semi-colon (\docenvdef{;}) to the end of the \textit{assignment} statement\sidenote{Again -- your personal choice as to whether to include spaces or not between the \texttt{C}, \texttt{=}, \texttt{'banana'}, and \texttt{;} (Maybe try it both ways to convince yourself at least in this context, spaces do not matter.)}, i.e.
\begin{docspec}
C = 'banana';
\end{docspec}
\noindent now does not results in anything being echoed to the command line (but the \textit{Workspace} is still updated to reflect this variable assignment). 
%------------------------------------------------
...
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item \docenvdef{exponentiation} --- \(^{\wedge}\) --- raises one number of variable to the power of a second, e.g. a\(^{b}\), a to the power b, which is written in MATLAB as \texttt{a\(^{\wedge}\)b}.
\item \docenvdef{multiplication} --- \(\times\) --- e.g. a\(\times\)b, written in MATLAB as \texttt{a\(\ast\)b}.
...
The order in which the numerical operators are written down is important and will execute them in a specific order (operators higher up the list, executed first), i.e. first \texttt{\textasciicircum}, then \texttt{\(\ast\),/}, and last \texttt{+,-}. There is also \textit{negation}, when you change the sign of a variable, and which is executed  immediately after exponentiation. The assignment operator (\docenv{=})\sidenote{This is \textbf{NOT} 'equals to'.} comes last. \uline{If you are unclear about the order numerical operators are carried out, then place parentheses \texttt{()} around the component of the calculation you wish to be carried out first to enforce a particular order} (this can also help in making an equation easier to read and ultimately, easier to debug code). For example, consider:
...
We will see more of \textit{relational and logical operators} later when we start to get into some proper coding. For now, you only need to know that a relational operator is one of:
...
\noindent and  test the relationship between 2 variables. Note in particular, that the \docenv{equality} symbol (that tests the equivalence between two variables) is represented by TWO \texttt{=} characters\texttt{ (==), }and remember that a single \texttt{=} character is the \docenv{assignment operator}.
...
For now -- simply keep mind the existence of \textit{relational and logical operators} and what they look like.

%---------------------------------------------
%
\subsection{Functions (built-in)}

\textbf{MATLAB} provides numerous built-in \docenvdef{functions}\sidenote{We will be constructing our own later, at which point it should become apparent that there is nothing particular special about them.}. These \docenv{functions} have specific names assigned to them, so care needs to be take not to give a variable the same name as a function to avoid getting confused further down the road.
Giving an exhaustive list (and brief description) is outside the scope of this document\sidenote{A full list of functions can be found in the MATLAB Help Documentation under \textit{functions}.}. Common \docenv{functions} will be progressively introduced as this text  progress. Note that in addition to the on-line Help documentation, information on how to use a function and example uses is provided by typing \texttt{help }and then the function name (separated by a space) at the command line.

MATLAB also provides several built-in mathematical constants (saving having to define a variable with the appropriate number). This are simply variables that have been already defined and assigned values, but which  you cannot change (hence the term 'constant'). For instance,  the value of \(\pi\), is assigned to a built-in variable with the name \docenvdef{pi}. You can access (display) its value  by typing its name at the command line:
...
In this example, the  use of the \textit{function} is rather trivial -- you need to tell the \docenv{pi} absolutely nothing, and it spits back the same thing (the value of \(\pi\)) each and every time. In most other \textit{functions}, you will find that you have to pass some information, and the return value will depend on the input. (This will all become apparent in due course ...)  
%---------------------------------------------
...
\item \docenvdef{clear all} --- (Closes all figure windows.)
\item \docenvdef{exit} --- Exits \textbf{MATLAB} and hence enables additional drinking time in the bar.
\end{itemize}
...
So far, your variables have all be what are known as \textit{scalars} -- i.e. single numbers (or strings).
One of the most powerful things about MATLAB is its ability to represent vectors (1D columns or rows of numbers or strings) and arrays -- 2D and higher dimensional regular grids of numbers or strings. (\textit{matrix}\sidenote{Not to be confused with the film starting Keanu Reeves.} is the name commonly given to a 2-D array.)
...
\marginnote[-0.5in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
The \docenvdef{colon operator} can be used to much more rapidy create vectors (as long as the elements form a simple sequence in value) as compared to typing in the list of values explicitly. There are two variants to the syntax:
\begin{docspecmargin}
A = j:k
\end{docspecmargin}
and
...
\\In the second example, \texttt{i} is the increment \textbf{MATLAB} will use to complete the sequence from \texttt{j} to \texttt{k}. In the example in the text, you could have created the array \texttt{B} by typing: 
\begin{docspecmargin}
>> B = 0.5:0.5:2.5
\\B =
\\ \ \ \ \     0.5000    1.0000    1.5000    2.0000    2.5000
\end{docspecmargin}
(More commonly, you might place the colon operator and its min/(/increment)/max values inside a pair of brackets, i.e. \texttt{A = [0:3].} so that it is unambiguous that you are creating an array
\end{mdframed}}

Vectors are 1-D arrangements of numbers (or strings).  You can enter them into MATLAB as a list of space-separated value, encased in (square) brackets, \texttt{[ ]}, e.g. 
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item Display the contents of the vector in the command window by typing its name at the command line. Obviously, this will quickly become useless for very large vectors\sidenote[][-0.0in]{Try creating a vector from 1 to 100,000 and then displaying it ...}.
\item Refer to the Workspace window, although this also ends up a total Fail for long vectors.
\item Use the \docenv{length} or \docenv{size} function (see Box). 
\item Refer to the \textit{Workspace window} ... but ... by default, the \textsf{Size} of variables is not one of the displayed columns (instead, it has to be added from \textsf{Choose Columns} right-mouse-button-click menu item). 
\end{enumerate}
...
length(A)
\end{docspecmargin}
returning its integer length, and which could in turn be assigned to a variable, e.g. \texttt{B = length(A)}. (Technically, \docenv{length} returns the largest dimension of an array.)
\end{mdframed}}

\marginnote[-0.0in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{size} (use \#1)
\\Returns both dimensions, even though for a vector, one of them always has a value of \texttt{1}. This does allow you to determine its orientation though, as for the example of \texttt{A = [1:10]}: 
\begin{docspecmargin}
>> size(A)
\\ans =
\\ \ \ \ \ 1    10
...
If you find that you want a different orientation (row vs. column) of the a vector, the vector can be flipped around (converting row-to-column and column-to-row) using the \textit{transpose operator} (\texttt{.'}), e.g.:

\begin{docspec}
D = B'
\end{docspec}

\noindent will turn the vector \texttt{B} into one (assigned to the variable \texttt{D}) with he same orientation as \texttt{C}. \sidenote{Note ... MATLAB gives the syntax as \texttt{.'}, whereas I always only ever added the \texttt{'} bit ... which works ...} You can also use the \docenvdef{transpose} function.

You can also re-order the values in a vector (hence addressing the restriction in using the colon operator to create a vector that the values must be monotonically increasing rather than decreasing). Depending on the orientation of the vector, you can use either the \docenvdef{flipud} (for column vectors), or \docenvdef{fliplr} (for row vectors), functions to re-order the elements.
...
As well as reading out an existing value of a vector, you can also replace an existing value by assigning the new value to the appropriate index position. e.g. to replace the first element
with a value of \texttt{0.0}:
\begin{docspec}
B(1) = 0.0
\end{docspec}

\noindent (Here, you are saying that you would like to assign the value of \texttt{0.5} to the element in the vector given by the index \texttt{1}. The previous content of the array at index position \texttt{1} is simply over-written.)
...
You can access more than a single element of a vector at a time, by means of the \docenvdef{colon operator}, \texttt{:} to define a min, max range of indices. For example:  
\begin{docspecmargin}
>> B(2:4)
\\ans =
\\   1.0000
...
So far ... I suspect this is heavy-going and there is a lot to try and remember, such as command names, although knowing just that certain commands exist, is enough to start with and \textbf{MATLAB} Help can be used later tot find out the exact name (and usage syntax). All this, and we have not  even  gotten on to matrices (2-D arrays) yet ... So, we'll take a diversion to look at some basic plotting techniques that will  make sense now that you can create vectors of numbers to plot (and later, important some 'real' data). Unless you have forgotten how to create vectors already ... :(

%------------------------------------------------

\subsection{Plotting}

First -- create yourself a dummy dataset to plot. You are going to need to create yourself a pair of vectors -- these can have any values (all numbers though) in them that you like, but perhaps aim for 1 vector with values counting up from 1 to 10 (or similar) -- this will form your \textit{x}-axis, and the 2nd column ... whatever you like. \sidenote[][-3.00in]{Looking ahead -- you could create a y-axis vector formed of the squares of the numbers in the x-axis vector: 
\begin{docspecmargin}
>> Y = X.\(^{\wedge}\)2  
\end{docspecmargin}
(The \texttt{.\(^{\wedge}\)} bit says to square the value of each and every  element in the vector.)}
The command \docenvdef{figure} creates a figure window, which is where MATLAB displays its graphical output ... but on its own, without anything in it ... useless. So, lets put something in it, with the simplest possible graphical way of displaying data called \docenvdef{plot}. 
...
With any new \textbf{MATLAB} command (\textit{function}), get int the habit of looking up the \texttt{help} text first (also refer to alternative/simplified help provided in this text). The key information that will get you started appears at the very top of the text that \texttt{help} returns on \texttt{plot}:

\begin{docspec}
PLOT(X,Y) plots vector Y versus vector X.
\end{docspec}

\noindent This tells you that you need to pass to \texttt{plot}, your \textit{x}-axis data vector (by its variable name), followed by your y-axis data vector (by its variable name) -- comma separated. Do this, and depending on just what or how random your \textit{y}-axis data was, you should end up with something like Figure \ref{fig:plot-basic} in a window captioned "\texttt{Figure 1}".\sidenote{If you cannot see the figure window ... check that the window is not hidden behind the main \textbf{MATLAB} program window!} 

This ... is easily the least professional plot ever (aside from anything at all created in \textbf{Excel}). And one that breaks all the most basic rules of scientific presentation, such as an absence of any labelling of axes. There is also no title, although here in the course text I have added a figure caption in the document so I can sort of get away with it. This is the default output of the basic \texttt{plot} function and you'll just have to deal with it (i.e. add a series of commands to add missing elements of the \texttt{plot}).
...
Note that by default, \textbf{MATLAB} also scales both axes to reasonably closely match the range of values. In the example here, the default min and max
...
\item You might want to specify the axis limits. The command is \docenvdef{axis} and it takes a vector of 4 values as its input -- in order: minimum x, maximum x, minimum y, and maximum y value. e.g. \texttt{axis([0 10 -100 100])} would specify an \textit{x}-axis running from 0 to 10, and a \textit{y}-axis from -100 to 100.
\end{enumerate}

Information as to how to use all of these commands can be found via MATLAB \texttt{help}. But a typical sequence, that gives rise to the improved plot shown in Figure \ref{fig:ch1-plot-basic2}, is given in the margin.
...
You can also have more than one plot in a single Figure window. As an example, create some sine waves using the \docenvdef{sin} function (see \texttt{help}) over the range 0 < \textit{x} < 2\(\pi\), e.g.:
\begin{docspec}
>> x = 0:0.1:2*pi;
\\>> y = sin(x);
\\>> y2 = sin(2*x);
\end{docspec}
(Note how in the first line, the colon operator is used to create an \textit{x} vector from 0 to 2\(\pi\), in steps of 0.1. The second and third lines calculate the sine of all the x values, and sine of 2 times the \textit{x} values, respectively, and assign the results to a pair of new vectors, \texttt{y} and \texttt{y2}.)
...
You might just want to save the figure. (Why create it in the first place in fact if you are just going to throw it away ... ?) Again, you can do this via the GUI or at the command line
\sidenote[][-0.25in]{
...
\\\noindent where \texttt{FILENAME} is the filename as a string or a variable containing a string.
}.
From the GUI, you have the option to save the figure in a way that can be loaded later and re-edited -- this is the \texttt{.fig} format option. Or you can save (export) in a variety of common graphics formats (although once saved in this format, the graphics can only be edited later using a graphics package).
...
\\ \texttt{>> close}
\\ \noindent To close all currently open Figure windows:
\\ \texttt{>> close all}
\end{mdframed}}
...
To enter the elements of a matrix, there are only a few basic conventions:
\begin{itemize}[noitemsep]
...
\end{docspec}
\noindent MATLAB then displays the matrix you just entered\sidenote{Remember that you can add an \texttt{;} to the end would prevent the results of the variable assignment being displayed in the \textit{Command Window}.}:
\begin{docspec}
A =\\
    15   7  11   6\\
    13   1   6  10\\
    ...
    5   15  20   9
\end{docspec}
\noindent Once you have entered the matrix, it is automatically remembered in the MATLAB workspace. You can refer to it simply as \texttt{A}.

Now go find the array you have just created in the \textit{Workspace window}. Double-click on its name icon and see what goodies appear on the screen. This is a fancy array editor which looks a bit like one of those dreadful \textsf{Excel} spreadsheet things. You can see that this might be handy to edit, view, and keep track of at least moderate quantities of data. This is a useful facility to have. However, we are going to concentrate on the command-line operation of \textbf{MATLAB} in this class because that will give you far more power and flexibility in applying numerical techniques to problem solving, and will form the basis of scripting (computer programming by another name) that we will see in a few lectures time. Close down this nice toy to leave just the original windows.
...
\\{\Large rows, columns}
\hfill \break
\\\noindent(You can always create a test matrix and access a specific element to check if in doubt!)
...
\marginnote[-0.25in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
You can also determine the shape of your array using the \docenvdef{size} function. For a 2D array (matrix), when you pass it the name of your array, it returns the number of rows followed by the number of columns (in that order). 
\end{mdframed}}
...
You can treat vectors and matrices (or parts of vectors and matrices), mathematically, as you would treat single values (i.e. \textit{scalars}) but unlike a scalar, the transformation is applied to all specified elements of the array. This applies for all the basic numerical operators\sidenote{Technically ... or at least to be consistent with other operations, you might write multiplication as \texttt{.*} rather than just plain old \texttt{*}. The preceding dot tells MATLAB not to treat this as matrix multiplication but to carry out the operation on each element in turn. In this case, it is the same thing (and both notations work the same), but later, is not. (This will make more sense when you get to see it in action, later.)}. For example, for vector \texttt{B} in the earlier example,
\begin{docspec}
>> 2*B
\\ans =
\\ \ \ \ \ 0     2     3     4     5
...
The \textit{function} \docenvdef{sum} ... sums things. The MATLAB Help documentation (\texttt{help sum}) says:
...
\end{mdframed}}
? (Sure -- you also do it by using a calculator, but you will not always have such a small data-set as here. Perhaps you'll get a much larger data-set in the assessed exercise ;) So, practice doing it properly.) The \textbf{MATLAB} function for this is \docenvdef{sum}.

\newthought{Question:} What is the sum of the 2nd row of \texttt{C}?
 For a matrix (rather than a vector) as input, \docenvdef{sum}  returns the individual sums of each column, and so on its own;
\begin{docspec}
>> C\\
C =\\
      255   119   187   102\\
...
ans =\\
      918   680   714   476\\
\end{docspec}
\noindent gives you a row vector consisting of the sums of the individual columns of the matrix \texttt{C} above.

This is where the \docenvdef{transpose} function (\texttt{'}) comes in handy (see earlier). In this case, it flips a (2D) matrix around its leading diagonal (columns become rows, and rows, columns)\sidenote[][-1.0in]{This is almost true. Technically the function you want is \texttt{.'}, as \texttt{'} will change the sign of any imaginary components. For real numbers, they are the same.}
...
\\\noindent \docenvdef{flipup} -- flips the matrix in the up/down direction
\\\noindent \docenvdef{fliplr} -- flips the matrix in the left/right direction
...
ans =\\
      ...
      102   170    51   153
\end{docspec}
\noindent (transposing the matrix turns the rows into columns)
\begin{docspec}
>> sum(C')\\
ans =\\
      663   510   782   833
\end{docspec}
\noindent Now you get a row vector consisting of the sums of the individual columns of the matrix C, but since you have transposed the matrix \texttt{C} first, these four values are actually equal to the row sums.
...
ans = 663\\
      ...
      833
\end{docspec}
\noindent to give you  a row vector format that corresponds to the rows  of the original matrix \texttt{C}.
\sidenote[][-0.25in]{Note how you can combine multiple functions in the same statement to create \texttt{sum(C')'}. However, to start with, it is much safer to do each step separately and hence be sure what you are doing.}

Finally, if you wanted the sum of *all* the elements in the matrix \texttt{C} in the example above, you could sum all the columns to give you a row vector of partial sums, and then sum the elements in the row vector to give you the grant total sum of all the elements. You can do this, either in seperate steps:
\begin{docspec}
>> D = sum(C);\\
>> E = sum(D);
\end{docspec}
...
We will not concern ourselves overly with multiplying vectors and matrices together ... but you should be aware that \textbf{MATLAB} can do matrix math. For now, it is worth nothing the difference between \texttt{*} and \texttt{.*} operators in the context of arrays. For example, consider 2 vectors, A and B:
...
To multiple the elements of A and B together \uline{pair-wise}, use .*:
...
while for matrix multiplication, \(\begin{pmatrix}1 & 1 \\
2 & 2 \\
...
There are a number of different ways to load/import data into the \textbf{MATLAB} \textit{Workspace}. Rather than try and tediously list and describe the commands and syntax and blah blah, we'll be going through a couple of (hopefully) slightly less tedious data-based examples as we progress through the course text. In this way, if nothing else, you might accidently learn some science even if nothing much about \textbf{MATLAB} ...

%------------------------------------------------

\subsection{Where am I?}

Before anything -- you need to know 'where you are'. If the file you want to load in is not in the directory \textbf{MATLAB} us using, it will not find it. And if you save something and have no idea where it is being saved ... that can hardly go well.

\textbf{MATLAB} has a default directory that it starts up in and looks at first. For basic Windoz installations\sidenote[][-1.0in]{At installation, this directory can be specified and hence may not be this one. Also -- different operating systems will have different default locations.}, this directory is:
...
>> load(filename)
\end{docspecmargin}
where filename is the name of the file (remember: as a string, it needs to be enclosed in quotation marks). The file might be plain text (ASCII) or a \textbf{MATLAB} workspace file (see below), in which case it should have the file extension \texttt{.mat}. To force \textbf{MATLAB} to treat the file input as ASCII or a \textbf{MATLAB} workspace file, pass a second parameter (separated from the filename by a comma) -- \texttt{'-ascii'} for ascii, and \texttt{'-mat'} for a \textbf{MATLAB} workspace file. 
...
It is not necessarily to have all your files end up here, so there is a way to change the \textbf{MATLAB} directory that you are working in which work in a similar way to UNIX/LINUX for those of you who are familiar with navigating your way around these operating systems. You can change the directory that \textbf{MATLAB} is  working from by typing:
\begin{docspec}
>> cd DIRECTORY\_PATH
\end{docspec}
where \texttt{DIRECTORY\_PATH} is the path to the directory in which you want to work from and where you want your data files (and later, code files) to live.

Another alternative is to add a 'search path' (\texttt{addpath}) so that \textbf{MATLAB} knows of an additional place to look for files.  

\marginnote[-0.0in]{The command \docenvdef{addpath} will add a search path to the MATLAB workspace. e.g. \\\texttt{addpath DIRECTORY\_PATH} where \texttt{DIRECTORY\_PATH} is a \textit{string} (characters in between inverted commas) or name of a variable containing a string.}

There is also, of course, the GUI -- from the \texttt{File} menu the option \texttt{Import Data..}. will run the data import Wizard -- note that you might have to select \texttt{All Files (*.*)} from the file type option box in order to find the file. I'll leave you to work the rest out for yourselves ... Maybe try importing the data into \textbf{MATLAB} this way once you have done it successfully using the load function at the command line. The GUI can also be used to change the directory you are working from (duplicating the functionality of the \texttt{cd} command) and add paths to search (duplicating the functionality of the \texttt{addpath} command).

%------------------------------------------------

\subsection{Loading and importing data}

The simplest way (other than via the \textbf{MATLAB} GUI and the beautiful green \textsf{Import Data} icon) is to use the \docenv{load} function (see Box)\sidenote{There is also a much more flexible way of loading text-based data using the function \texttt{textscan}, but that also requires files to be explicitly opened and closed using \docenvdef{fprintf}. We'll see a little of this later.}.

As a brief exercise and practice using \texttt{load} -- first download the data file \texttt{etheridge\_etal\_1996.txt} from the course webpage\sidenote{\texttt{http://www.seao2.info/teaching.html}}. You might start by viewing the contents of the file by opening it in any text viewer (or \textbf{Excel}). This is always a good place to start as it enables you to see what you are getting yourself in to (i.e. format of the file, any potential formatting issues, approximate size and complexity of the dataset, etc). Then import the data into the \textbf{MATLAB} workspace using the  \texttt{load} command. Becasue the data is a plan text (ASCII) format and not a special MATLAB .mat file, you need to specify the format:

\begin{docspec}
>> load('etheridge\_etal\_1996.txt','-ascii');
\end{docspec}

Try simply typing the name of the variable that was automatically created (\texttt{etheridge\_etal\_1996}) or the one you chose, if you assigned the imported data to a specific variable name  (see Box) to provide a crude view of the data. To view the contents of the variable in the \textsf{Variables} window -- double  click on the name of the variable in the \textbf{MATLAB} \textsf{Workspace window.} This should open up a spreadsheet-like window in which the data can be viewed, sorted, and even edited.

For practice -- plot the data\sidenote[][-2.5in]{using \texttt{plot}}, remembering to label the figure appropriately\sidenote[][-2.5in]{ FYI: the first column of the data and x-axis is year, and the 2nd column of the data and y-axis is the mixing ratio of CO\(_{2}\) in air in units of ppm.}. If you just type \texttt{plot} and pass the (here: default) name of the data array:

\begin{docspec}
>> plot(etheridge\_etal\_1996);
\end{docspec}

... strange things happen (as per Figure \ref{fig:ch1-plot-icecore_default}). In fact, \textbf{MATLAB} is doing what \textbf{Excel} would  in a \textsf{Line Chart} with 2 columns of data selected -- rather than plot y (2nd column) vs. x (1st column), the values of both columns are plotted against row number.\sidenote[][-1.5in]{And this is why you should remember to use the \textsf{Scatter} (or \textsf{(X,Y)}) \textsf{Chart} in \textbf{Excel} for plotting (x,y) data.} Instead, if you remember, the format of the \textbf{MATLAB} \texttt{plot} function is:
...
\noindent So you need to specify each column of the data (i.e. each vector) separately. If you recal how to specify an entire row or column of an array, you should see that you need to type:
...
This is an equally valid way of doing things. It is longer ... taking 3 lines rather than 1, but the most important thing is to be happy that you understand what is going on. If breaking things down into multiple lines and creating new variables helps -- DO IT! Ultimately, you should end up with something like Figure \ref{fig:ch1-plot-icecore}.
...
Arrays of numbers can be saved in a plain text (ASCII format) by means of the \docenv{save} function in a simple reverse of the use of \docenv{load} (see Box). Try re-saving the ice-core data as an ASCII format
...
>> save(filename)
\end{docspecmargin}
which saves the entire workspace to a \texttt{.mat} file (with the filename given by the string \texttt{filename} (in quotation marks), and:
\begin{docspecmargin}
>> ...
...
The entire workspace (including all variables and their values, or just the values in a single variable if you wish) can be saved to a file and then later re-opened. The file format is specific to the \textbf{MATLAB} program and the file-name extension by default is \docenvdef{.mat}. You might find this very helpful to use in long lab exercise or large modelling projects, particularly if you do not come back to work at the exact same computer each time or wish to use continue the same piece of work on a laptop elsewhere. Try saving the current \textsf{Workspace}, closing down the \textbf{MATLAB} program. Re-running it, and then loading in your saved \texttt{.mat} file.
\sidenote{This sequence is going to look something like:
\begin{docspecmargin}
>> save mystuff
\\>> exit
\end{docspecmargin}
...
\section{Basic data processing}

As an example to kick-off some data-processing tricks, load in the Phanerozoic CO\(_{2}\) dataset: \texttt{paleo\_CO2\_data.txt}. You can just import it into \textbf{MATLAB} using the \docenv{load} function. However, there is a complication here -- unlike the ice core CO2 dataset, you now have 4 columns in the array\sidenote{Remember that you can diagnose its size with ... \texttt{size} (or refer to the Workspace window)}. The first column is age (Ma), the second the mean CO\(_{2}\) value, and the 3d and 4th are the low and high, respectively, uncertainty limits. Not forgetting in the space of 5 minutes (I hope!) how to reference specific columns of data in a matrix\sidenote{HINT: the \texttt{colon operator} (see earlier).} -- plot the mean paleo CO\(_{2}\) value as a function of age (in Ma). If you  closed the previous \textsf{Figure} window (see earlier), it is not essential to explicitly open one (using the \texttt{Figure} command) -- when you use the \texttt{plot} command, if there is no open \textsf{Figure} window, \textbf{MATLAB} will kindly open one for you. How thoughtful. The result should be something like \ref{fig:plot-CO2basic}. O dear ...
...
\item You could sort it in \textbf{MATLAB} using the GUI variable view window. But lets not cheat for now.
...
For another example: download the historical global temperature anomaly dataset: \texttt{647\_Global\_Temperature\_Data\_File.txt}\sidenote{\href{https://climate.nasa.gov/vital-signs/global-temperature/\#https://climate.nasa.gov/vital-signs/global-temperature/}{https://climate.nasa.gov/vital-signs/global-temperature/}}. The columns are: (1) year, (2) annual mean, and (3) 5 year (running?) mean. Plot (separately) both the annual mean, and the 5-year mean. \sidenote{plus labels, title, etc etc}

The 20th century average global temperature across land and ocean surface areas is apparently 13.9\degree C. So firstly, try changing the temperature anomaly data into absolute temperatures (by adding the 20th century global average value to all the data values), and then re-plot.

Next, convert the temperature units (of both annual mean and 5-year mean) -- from \degree C to \degree F. An approximate conversion is:
...
Create a new data array in \textbf{MATLAB}, with year as the first column (as per the original data) and the 2nd and 3rd columns as annual mean and 5-year mean temperatures in units of \degree F.\sidenote{Remember -- MATLAB will happily do the same to all values in a vector in an equation, as for a single scalar value in the same equation. Start by creating a new array with a single column, copied form the 1st column of the original array.} If it help -- play  the data conversion game in \textbf{Excel} first (e.g. creating new columns in a spreadsheet to firstly hold  absolute temperatures rather than anomalies, and then temperatures in \degree F rather than \degree C).
 
Re-plot (in \textbf{MATLAB}) the final temperature trends in \degree F.
...
\newthought{As an example} -- set \texttt{>> hold on}, and then plot the minimum and maximum CO\(_{2}\) values (columns \#3 and \#4) in different symbols and different colors, on top of your existing plot. If you want to then label what different lines or sets of points are, you can add a legend with the \docenvdef{legend} command. For instance you have managed to successfully plot the mean CO\(_{2}\) values as discrete black circles, and the minimum and maximum uncertainty limits as blue and red lines, respectively, you could call:
\begin{docspec}
>> legend('Mean CO\_2','Lower uncertainty limit','Upper uncertainty limit');
\end{docspec}
and it should end up looking like Figure \ref{fig:plot-CO2basic4}.
...
We'll stay with the Phanerozoic proxy (CO\(_{2}\)) data, but put a different (graphical) spin on it.

Consider ... \docenvdef{scatter}. In fact, don't just considered it, \texttt{help} on it. The simplest possible usage is, apparently:
\begin{docspec}
SCATTER(X,Y) draws the markers in the default size and color.
\end{docspec}
(where \texttt{X} and \texttt{Y} are vectors). This almost could not be more straight-forward.  Make yourself an \texttt{X} and \texttt{Y} vector out of the loaded-in dataset (or if you are feeling brave, you can pass in directly the appropriate parts of the dataset array), close the existing Figure window\sidenote{See earlier.}, and \texttt{scatter}-plot the (mean) CO\(_{2}\) data.
...
There are 2 different simple \textbf{MATLAB} commands for visualizing a 2D dataset (i.e. a matrix) as a bitmap image (and via a 3rd command, viewing various bitmap photo and image format files too). As something (2D data) to play with -- load in the matrix \texttt{model\_grid.txt}.
...
First off -- as before, view the data in the array viewer, just to get a feel for what you are dealing with here (although you are unlikely to be much wiser after doing so). So go ahead and employ the \docenv{pcolor} function in its simplest possible usage (see Box). You can see (Figure \ref{fig:ch1-plot-pcolor1} ) that it is ... something. Maybe a little like the continents, but up-side-down at the very least. What to do?

Well, it is a good job that you remember how to re-orientate arrays, right?\sidenote[][-0.5in]{You don't? See earlier in the Chapter ...} If you guess right first time (three different basic transformations of a matrix were described), you get Figure \ref{fig:ch1-plot-pcolor2}.
...
\marginnote[-0.5in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
\docenvdef{pcolor}
\\MATLAB claims that pcolor(C) plots; "a rectangular array of cells with colors determined by C. Actually, I believe MATLAB on this.
So if you have a matrix, MATLAB will plot a regular arrays of cells, with each cell representing one of the elements in the matrix, and will color that cell according to the value. (\texttt{pcolor} will by default, autoscale how the color scale maps onto the data in the matrix such that both extreme ends of the color scale are used.)
\end{mdframed}}

Next try something very similar. but using the \docenv{image} function. Now the model grid is the correct way around! I have absolutely no idea why and why it is reading the matrix dimensions differently from \docenv{pcolor}. I am sure you could Google and find out. But you would have to actually care first.
...
\subsection{Matrix math (systems of equations)}
...
Your wish is granted. In \textbf{MATLAB}, it is possible to store all of your commands in a single text file, and then request that they (the list of commands) are all executed (sequentially)  at one go. \textbf{MATLAB} gives this text file a fancy name (because it is a very fancy piece of software, after all) -- a \textit{script}\sidenote{The conception of a \textit{function}, will be introduced later.}, otherwise known as an \textit{m-file}.  To create a new \textit{m-file}; from the  File menu, select \textsf{Script} (a common type of \textit{m-file})\sidenote[][0.0in]{In order version of \textbf{MATLAB}: \textsf{File}/\textsf{New} menu, and select: \textsf{Blank M-file}.}. You will see a text editor (more fancy-ness) appear in front of your very eyes, containing your requested (but currently empty) \textit{m-file}. Save the \textit{m-file} to your directory of choice. Alternatively, simply create a new (blank) text file and saving it with the extension \textsf{.m}, rather than e.g. \textsf{.txt}, creates you a (script) \textit{m-file}. From an \textit{m-file}, you can issue all the \textbf{MATLAB} commands you previously would have entered individually, line-by-tedious-line, at the command line. Furthermore, having created and saved  a \textbf{MATLAB} script, it can be executed again and as many times as you like.
...
disp('STRING')
\end{docspecmargin}
where \texttt{STRING} is a .... \textit{string}, get the \textit{string} displayed as text at the command line.
\\You can also pass the name of a variable that contains a string, e.g.
\begin{docspecmargin}
disp(VARIABLE)
\end{docspecmargin}
where the contents of \texttt{VARIABLE} is a string.
...
The schematic structure of this program (\textit{script}) is shown in Figure \ref{fig:ch2-hello_world}.\sidenote{Note that not all of the comment lines are shown in the structure schematic -- only the main program summary at the top.}
...
\textit{Is it important to recognise that (1) bugs will not always be flagged by MATLAB with a line number, and you can have valid code but nonsensical results, and (2) the mistake is often made earlier in the code than when MATLAB flags up a problem line.} 
...
\noindent Now \textbf{MATLAB} does not like \texttt{function or variable 'k'} because it cannot find that it has ever been defined. Is \texttt{k} meant to be a function or variable? Look up \texttt{help plot} to remind yourself of the correct syntax if the problem is not immediately obvious.
...
A \textit{function} is defined (and differentiated from a \textit{script}) by a special line at the \uline{very start}\sidenote{Literally: line 1. Not even a comment line is allowed to appear before the \textit{function} definition line.} of the \textit{m-file} (see Box).
...
\\The all-important fancy first line of a \textit{function}, as defined in MATLAB help, looks like:
\begin{docspecmargin}
...
>> Y = sin2(0.5);
\end{docspec}
\noindent does the same but assigns the answer to the variable Y (with the semi-colon suppressing output to the command line).
...
Finally, it is important to note that by default, any variables created within a \textit{function} are TOP SECRET, and by that, I mean that they are not accessible to the main \textbf{MATLAB} workspace and do not appear listed in the \textsf{Workspace window}. To see that this is a non-Trump-able true fact, create the following function (basically, the first example but split into 2 steps):
\begin{docspec}
...
But before we worry about anything else (e.g. how to apply a \textit{conditional} statement), you'll  need to know about inputting information into a \textbf{MATLAB} program from the keyboard\sidenote{All programming languages have such a facility and man basic programs, at least in the Old Days prior to widespread GUIs, make use of keyboard input}. Amazingly, you can guess (I actually just did) the command for requesting input -- it is \docenv{input} (for 'input' -- a rare occasion when everything is logical and simple!) (see Box).
...
In the language of \textbf{MATLAB} syntax (see Box), the EXPRESSION is  whether the string contained in \texttt{MY\_ANSWER} is '\texttt{yes}'. How do we ask \textbf{MATLAB} to compare the value of \texttt{MY\_ANSWER} with '\texttt{yes}'?
...
You could also turn this around, and test for any answer except 'no' (the \(\sim\) is making the test, not 'no'), i.e.
\begin{docspec}
...
So -- go modify your code to allow for a 'YES' or a 'yes'. Hell, try allowing for  a 'Y' or a 'y' as well.\sidenote{Sort of for this reason and that there are many different ways of writing 'yes', software often requires you to answer 'yes' in a restricted number of ways -- this restriction is made clear as part of the message that asks the question. Common is to restrict the answer to 'Y' or 'y'.} (You could extend it to 'no' also but I think you get the point ...)
...
How many bananas could you eat in a day? I bet it is less than ten. We'll let the computer ask and if the answer is 10 or more, you (the computer) shouts: 'lier!'.\sidenote{This example is even more stupid than the last one. But no more stupid than in any computer programming textbook and it will at least demonstrate a subtly different usage of \texttt{if ...}.}

The basic code is very similar to before. Create a new \textit{m-file}, add a comment line, define your question ('How many bananas do you think you could you eat in a single day?') and then get \textbf{MATLAB} to ask it and pass back whatever is entered in at the command line. The only difference at this point -- refer to the usage of \texttt{input} (see earlier Box) -- is that we want a number input rather than a string. You can call the \textit{variable} into which you assign the result of \texttt{input}, the same as before, or to make it distinct, e.g. \texttt{N\_BANANAS}, i.e.
\begin{docspec}
...
A less commonly used alternative to \texttt{if ...} is \texttt{switch ... case ...} and is helpful in the case of multiple possible correct answers and/or multiple different answers.

\marginnote[-2.5in]{\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=gray!10]
...
\noindent where \texttt{MY\_ANSWER} is the name of a fruit entered in, in response to input, e.g.
\begin{docspec}
MY\_ANSWER = input('What is your favourite fruit?,'s');
\end{docspec}

Note that for a list of multiple possible value, \textbf{MATLAB} requires the list after \texttt{case} to be encased in \{\}. For a single answer, it would just be:
\begin{docspec}
\ \ \ case 'banana'  
\end{docspec}
...
Consider, or rather: create a new \textsf{m-file}\sidenote{Comment it!}, and add the code for the following loop:
\begin{docspec}
for n=1:10
\\end
\end{docspec}
...
\item Print the value of \texttt{n} 'properly'\sidenote{Although you can get away with just writing:
\begin{docspecmargin}
disp(n)
\end{docspecmargin}
}, e.g.
...
\\end
\end{docspec}
or you can tart this up even nicer by creating a string that provides more explicit information back to you, which is when you really need to use \texttt{num2str}, e.g.
\begin{docspec}
for n=1:10
...
Using the other (numerical input) form of \texttt{input} (see earlier), the code fo entering 2 numbers, one after the other, might look like this (although in practice, your code is full of helpful comments, right?):
\begin{docspec}
...
\\The only problem here ... is that MATLAB does not know what the very first value of x is -- i.e. the value before the loop start and that you then try and add \texttt{input(prompt)} to. The solution is to initialise the value of x before the loop starts, e.g.
\begin{docspecmargin}
x = 0;
\end{docspecmargin}
...
\noindent What do you expect to happen to the value of m? Add some \texttt{disp} statements and print out the values of \texttt{n} and \texttt{m} (from within the \textit{loop}), each time around the \textit{loop}. Was this what you expected? Why? What about the following?
...
Then create a variant of this \textit{function}, and have it return the sum, rather than display it. i.e. you \textit{function} will now take as input, the number of numbers you wish to input, and will return the sum of those numbers.

Alternatively, your program (as a \textit{script}), before the loop starts, could ask for the number of values to be entered, passing this to the variable \texttt{max\_count}, with the loop then looking exactly like the above. In both cases you are substituting a fixed number (e.g. 4) for a variable that might contain any number.
...
\item Converting a number value of a (count) variable to a string (the \docenv{num2str} function), so instead of hard-coding in the string representing a number (\texttt{'1' in thsi example)}, you convert from the value of a counter, e.g. \texttt{num}2str(month).
\end{enumerate}
...
\noindent Save and run the \textit{script}. Satisfy yourself that you know what it is doing. Can you see that you are now automatically generating all the 12 filenames in sequence? And this only takes 3 lines of code total (not including the debugging line), compared with 12 lines if you had to write down all the 12 file names long-hand.
 
Now \textit{comment out} (or delete) the \texttt{disp(filename)} line, and add a new line to load in each dataset from the filename that is constructed each time the loop goes around.\sidenote[][-1.0in]{Remember that the load line goes inside the loop. (Why? Try writing it outside the loop (at the end) and see what happens if you like.)} This is almost identical to the use of the load command earlier in the example:
\begin{docspec}
temp(:,:,month) = load(filename);
\end{docspec}
\noindent but ... with 2 important differences. Firstly, rather than specifying the 1st, then 2nd, etc layer of the 3D array, are are specifying the layer with an index equal to \texttt{month}, which remember, counts up from 1 to 12 in the \textit{loop}. Secondly, rather than specifying the filename explicitly in the load command, we are passing the string contained in the variable \texttt{filename}. Hopefully on the previous line of code within the loop, you have created the string value of \texttt{filename} ...

Assign the  2D data array that is loaded in, to the \texttt{temp} array variable, \uline{at the next layer number}. Take a look at the \textsf{Workspace window} -- note that you have an array (\texttt{temp}) that has size 94\(\times\)192\(\times\)12. If temp is 94\(\times\)192\(\times\)1 then go back a page or so and go through the bit about loading data into a 3D array. You want to avoid over-writing the information that is already there, so the line;
\texttt{temp = load(filename)};
will not work (and you will only get a 94\(\times\)92 array after going 12 times around the \textit{loop}). Why? (Again, look back a page-ish.)\sidenote[][-1.0in]{If you are still stuck, then stick up a paw.}

At the end of (but still within) the \textit{loop} (i.e., before the \textit{loop} has completely finished), create a new \textsf{figure window} on one line, then plot (using \texttt{pcolor}) the monthly temperature data on the next line, and add the essential labelling stuff (lines after that). All within the loop still. This line should look something like:
\begin{docspec}
pcolor(temp(:,:,month));
\end{docspec}
and should produce extremely exciting graphics as in Figure \ref{fig:ch3-pcolortemp}\sidenote[][0.0in]{The 2D graphics will get *much* better later -- one thing at a time!}. (\uline{Don't} just type this line in blindly (maybe it doesn't work anyway). Make sure that you understand what you are doing (otherwise why do GEO111 at all?).)
...
Save and run the \textit{script}. Do you have 12 different temperature plots on the computer screen?\sidenote[][1.75in]{If not, stick you paw up in the air for help ...} Note that if you keep running the program, you'll get 12 more figure windows each time. This is where the \texttt{close all} command comes in useful, and you could add this at the start (or end) of your \textit{script}. Because if you re-run the \textit{script}, you wont then end up with 24 figure windows. And then 36 the time after that, and ... 
...
Finally ... look up \textbf{MATLAB} \texttt{help} on \docenvdef{getframe}. Then go back to your global temperature loading/plotting script and add the following line\sidenote{Where to put the line? See the \texttt{Example} given in the \texttt{help} on this function. It is exactly what you are doing here.}:
\begin{docspec}
M(month)=getframe;
\end{docspec}

Save and run. When \textbf{MATLAB} is all done, at the commend line type in:
\begin{docspec}
>> movie(M,5,2)
\end{docspec}
...
\marginnote{\begin{mdframed}[backgroundcolor=gray!5, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=gray!5]
...
\uline{Before} you move -- go look at your \textit{script} -- is it well commented? Would you be able to tell exactly what it does it by the end of GEO111? What about next year? Are the \textit{loop} contents indented? It is important that it is commented and laid out adequately.

Finally -- there are \textbf{MATLAB} commands to turn your \textbf{MATLAB} format movie into a format you can use elsewhere. The old ... and maybe still working (or not) command was \docenvdef{movie2avi} (see Box). The new/replacement command is \docenvdef{VideoWriter}. TO use this new \textbf{MATLAB} \textit{function}, adjust your code as follows and as per the \textbf{MATLAB} \texttt{help} on \docenv{VideoWriter}. We could also simplify things by not creating a 3D array, but rather over-writing a 2D array each time (having first created the animation frame).
...
No surprise that you might combine both \textit{loops} and \textit{conditionals} in the same programming structure. In fact, this becomes very powerful and is an extremely common device in programming. 
...
\\ \ \ \ if strcmp(my\_string,'n')
\\ \ \ \ \ \ \ \ break
\\ \ \ \ end
\\end
\end{docspec}
where\texttt{1000000} is simply chosen as a 'very large number' and one rather larger than the maximum number of numbers you could ever imagine entering\sidenote{There us a better way of doing this, with the \texttt{while} construct, that we'll see shortly.}.

The key new command here is \docenv{break}. The way the code works (hopefully!) is that at the start of a new iteration of the \textit{loop}, the 'another input' question is asked -- if no further input is required, the loop exits via the \texttt{break} command. Otherwise (the \texttt{else}), the user is prompted for another input. Note that now we have loops and conditionals nested together, it helps even more to \textit{indent} the code\sidenote{\textbf{MATLAB} will do this for you if you click on the Indent icon. It will also indent the code as far as it reasonably can, as you type.}. Also note that here -- the two different questions (demands) outputted to the screen -- 'Another input (y/n)?' and 'Please enter a number' -- are pre-defined before the loop starts. These same lines could be placed within the loop, but re-defining the variable e.g. \texttt{my\_question1} as 'Another input (y/n)?', each and every time, is redundant (i.e. it could instead simply be defined once at the start of the program). Also also note that in this code, the number entered in is assigned to the variable \texttt{my\_number} rather than \texttt{n} as was used before -- simply to help distinguish the number input from the string input (assigned to my\_string).

It is up to you to 'do' (i.e. add or modify the code) something with the number entered in an stored in the variable \texttt{my\_number}, as each time around the loop, the previous value is over-written by the new input.
...
\newthought{A more practical example} would be when saving a data file, to test for a filename already that already exists and if so, automatically modify the new file name so as not to over-write the file.\sidenote{Note that while in the \texttt{m-file} Editor, \textbf{MATLAB} asks you if you want to over-write an existing file, when saving a file directly from a program, no such dialogue box or warning is given.} The relevant function is \docenv{exist}, and in the case of a test for a file, returns either \texttt{0} (the file does not exist in the \textbf{MATLAB} search path, although that does not rule out it existing somewhere else entirely{}), or \texttt{2} (the file exists).

Clearly(?), in the example of saving the movie file (using the \texttt{movie2avi} command), you might well want to test whether the filename that you have chosen already exists (i.e. the value returned by \texttt{exist} is \texttt{2}). If so (i.e. the file exists), you need to modify the filename  by means of a new concatenation, perhaps appending something like '\texttt{\_NEW}' to the end of the string\sidenote[][-0.75in]{Recall that in using the \texttt{movie2avi} command, you pass a filename -- simply modify the filename passed, in a similar way to in which you modified the filename for loading the temperature data.}. If not, and the filename has not already been used, you can proceed as before -- the equivalent of 'doing nothing'. Go ahead -- try it (i.e. modify your code to avoid over-writing an existing filename).
...
\end{docspec}
Finally, using an \texttt{if} statement, test whether the value of \texttt{filename\_check} is equal to 2. If so, you are going to need to modify the filename string (\texttt{my\_filename}). If not, you can let the \textit{conditional} just \texttt{end} and proceed to saving. Modifying the filename is just as per for the example of loading global temperature distributions, e.g.
\begin{docspec}
my\_filename = ['NEW\_' my\_filename];
\end{docspec}
where here, we take the string contained in \texttt{my\_filename}, we append a 'NEW\_' to the start\sidenote{Note that because the filename already has its \textsf{.avi} extension attached, you'll have to modify the start of the string.}, and assign the new (longer) string back into the variable \texttt{my\_filename}.

The file naming becomes a little awkward, so rather than the entire filename + extension, you might just store just the filename in the (\texttt{my\_filename}) variable. i.e.
\begin{docspec}
my\_filename = 'GEO111\_movie'
\end{docspec}
but the remembering when you test fo rthe existence of a particular file, you must add the extension, i.e.
\begin{docspec}
filename\_check = exist([my\_filename '.avi'],'file')
\end{docspec}
(here we create a new string \texttt{[my\_filename '.avi']} by concatenating my\_filename with the extension '.avi'). If the filename exists, the new filename we generate can then be:
\begin{docspec}
my\_filename = [my\_filename '\_NEW'];
\end{docspec}
...
We can re-frame the earlier example programs using the \texttt{while} construct rather than the \texttt{for} loop. But now ... you need to specify under what conditions the loop continues as the basic syntax (see earlier or \textbf{help}) is:
\begin{docspec}
while STATEMENT (IS TRUE)
\\ \ \ \ CODE
\\end
\end{docspec}
Here -- \texttt{STATEMENT (IS TRUE)} is the conditional. For instance and rather trivially, create the following as a program and run it\sidenote{You ... are going to need a \textbf{Ctrl-C} on this one ...}:
\begin{docspec}
while true
\\ \ \ \ disp('sucker')
\\end
\end{docspec}
What has happened is that true is always ... true. Hence the condition is always met and the while loop loops forever. Conversely, \texttt{while false} would never loop, not even once. more interesting and useful is when the statement might change in value as the loop progresses.
...
\\end
\end{docspec}
This also will loop for ever as \texttt{n} is initialized to \texttt{0} and hence the statement \texttt{(n < 10)} is always true. But if we increment the value of \texttt{n} each time around the loop: 
\begin{docspec}
n = 0;
\\while (n < 10)
\\ \ \ \ disp('not a sucker')
...
\\end
\end{docspec}
then the loop will execute exactly 10 times (just as per \texttt{for n = 1:10}). You could also do this in reverse: 
\begin{docspec}
n = 10;
\\while (n > 0)
\\ \ \ \ disp('not a sucker')
...
\\end
\end{docspec}
Now, \texttt{n} counts down from \texttt{10} and when it reaches a value of \texttt{0}, it is no longer greater than zero and the statement (\texttt{n > 0}) is false (and the loop terminates).

It is not always completely obvious whether even simple while loops like this execute 9 or 10 (or 11) times particularly when often you might come across \texttt{while (n >= 0)} that allows the loop to continue when when \texttt{n} has reached z value of zero (but not below). So -- spend a little while playing about with different \texttt{while} configurations and loop criteria.

Finally, note that the conditional statement in the while loop need not test for an integer being larger or smaller than some threshold. One could equally loop on the basis of a string equality/inequality. For example, taking the previous example using \texttt{break} could be re-coded with a \texttt{while} loop:
\begin{docspec}
my\_question1 = 'Please enter a number: ';
\\my\_question2 = 'Another input (y/n)? ';
\\my\_string = 'y';
...
\\end
\end{docspec}
and ends up a slightly shorter and more compact piece of code, omitting the need for a \texttt{break} or a nested structure. However, in this example, we do need to initialize the value of \texttt{my\_string} (to 'y' -- assuming that we want at least one number). Try it and then adjust it so that the loop proceeds as long as the answer is not 'n' (rather than as long as it is 'y')\sidenote{See earlier Example.}. Note that as before -- it is up to you to 'do' (i.e. add or modify the code) something with the number entered in an stored in the variable \texttt{my\_number}, as each time around the loop, the previous value is over-written by the new input.
...
\newthought{Extending the filename checking example ....}\sidenote{Which first time around did not actually combine loops and conditionals in the same structure. Rather, a loop came first in the program (loading in and plotting the temperature data), ended, and only then a conditional checking the filename.} to fully integrate a \textit{loop} and \textit{conditional}. The problem with the previous code is that you checked for the existence only a default filename (and appended '\textsf{\_NEW}' if a file already existed).

One (partial) solution would have been, rather than append a pre-defined string ('\textsf{\_NEW}') to the filename, would be to request that the user provide either a string to append, or a completely new filename. You have already see the \docenv{input} command in action, so you should be in a good position to code this modification up.\sidenote{Effectively, all you have to do, if \docenv{exist} returns a \texttt{2} and the file already exists, is to ask for an alternative filename, and use the string entered in as the new filename (and don't forget to add the '.avi' extension to the end when saving)}

A better solution (because even when asking for an alternative filename -- what if that file exists too?) would be to keep checking for a filename clash and keep asking for a new filename, until a unique one is found. Who knows how many attempts this might take (to find an unused filename), so \texttt{while ...} would be a better choice of loop than \texttt{for ...}. Becasue \docenv{exist} returns a \texttt{2} if the file already exists, a logical condition for \texttt{while} would be while \docenv{exist} is returning \texttt{2}:
\begin{docspec}
my\_question = 'Please enter an alternative filename (without the extension): ';
...
Within the loop, a new filename is requested and then checked against the directory contents. What is missing is the initial value of \texttt{filename\_check}. In a previous example, we simply set a value at the start. If we did that here, the first line of this code would look like: 
\begin{docspec}
filename\_check = 2
\end{docspec}
In this case, we do not need a default filename as the user provides the very first filename that is tested. Alternatively, we could perform a single check before the \textit{loop} starts:
\begin{docspec}
my\_question = 'Please enter an alternative filename ...
...
\newthought{Looping through arrays.} In plotting e.g. global temperature distributions, it would be nice to add on the continental outline. Currently, and particularly with the very  basic 2D plotting you have seen so far (\docenv{pcolor}), you are left to some extent guessing where the land and where the ocean is.
...
(\texttt{continental\_outline\_start}), and ends on row \texttt{100} (\texttt{continental\_outline\_end}). The 2nd shape starts on row \texttt{101}, and ends on row \texttt{200}. etc etc
...
How? First you need to write the \texttt{plot} command in a more general form -- you do not want to have to read the values out of the \texttt{continental\_outline\_start} and \texttt{continental\_outline\_end} files manually. Hopefully, it should be apparent that you can re-write the plot statement for the first fragment, as:
\begin{docspec}
...
\item First, you need to 'open' the file -- the command (function) for this is called \docenv{fopen} (see Box). You need to assign the results of this function to a variable for later use.
...
\item Secondly ... you can read the content in (finally!). The complications here include specifying the format of the data you are going to read in. You also need to tell \textbf{MATLAB} the ID of the file that you have opened (so it knows which one to read from). The function you are going to use to do this is called \texttt{textscan}.

\item Close the file using \docenv{fclose} (see Box). You are going to have to pass the ID of the open file again when you call this function (so \textbf{MATLAB} knows which file to close).
...
\newthought{As an initial Example} to illustrate this alternative (and more flexible) means of importing of ASCII data, we are going to return to the paleo atmospheric CO\(_{2}\) proxy dataset file -- \texttt{paleo\_CO2\_data.txt}\sidenote{The version that you have used before -- not to be confused with a version ending in \texttt{.dat} that we will look at shortly ...}. Assuming that you have already (previously) downloaded it,  open it up in a text editor and view it -- you should see 4 neatly (ish) aligned columns of numeric values ... and nothing else\sidenote{This 'nothing else' is important as it is the reason why you were previously able just to \texttt{load} the data.}.

OK -- so having seen the format of the  data in the ASCII file, you are going to work through the following steps\sidenote{You can start off working at the command line if you wish, but ultimately, you are going to need to put everything into an \textbf{m-file}.}:

\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}

\item First 'open' the file -- you will be using the function command \docenvdef{fopen}, and passing it the filename\sidenote[][-0.0in]{For convenience, you could assign the filename (+ its path) to a (string) variable and then simply pass the variable name -- remember, no \texttt{' '} needed for a variable naming containing a string (whereas \texttt{' '} is needed for the string itself).} (including the path to the file if necessary). So that you can easily refer to the file that you have opened later, assign the output of \docenv{fopen}\sidenote[][-0.0in]{The output is a simple integer index, whose value is specific to the file that you have opened.} to a variable, e.g.
\begin{docspec}
>> openfile\_id = fopen('paleo\_CO2\_data.txt');
\end{docspec}

\item Now ... this is where it gets a  trickier -- the function you are going to use now is called \docenv{textscan}. Refer to \textbf{help} on \docenv{textscan}, but as a useful minimum, you need to pass 3 pieces of information:
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item The ID of the open file (you have assigned this to a handy variable (\texttt{openfile\_id}) already.)
\item The \textit{format} of the file (see margin note).
\marginnote[-0.0in]{\begin{mdframed}[backgroundcolor=light-gray, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
According to \textbf{MATLAB help}:
...
\end{mdframed}}(This is where it gets much less fun, but hang in there!) You simply list, space-separated, and between a single set of quotation marks, one format option per element of data.
\\In this particular Example, there are 4 items of data (per row) -- each of them is an integer or a floating point number\sidenote{At least, none of them are clearly strings, right?}, depending on how you want to look at it. Assuming that the data is a floating point number, the \textit{format} for the input of each number item, is \texttt{\%f}.
\end{enumerate}
...
\item Actually, it does get darker before the light at end of the tunnel ... what \docenv{textscan} actually returns, i.e. your  read-in data, is placed into an odd structure call a \textit{cell array}. It is not worth our while worrying about just what the heck this is, and if you view it in the \textsf{Variables} window (i.e. double click on the \texttt{cell array} name in the \textsf{Workspace} window), it does not display the simple table of 4 columns of data that maybe you were expecting. For now, we can transform this format into something that we are more familiar with using the \docenvdef{cell2mat} function, e.g.
\begin{docspec}
...
\item First 'open' the file as before (\docenv{fopen}) and assigned the ID returned by the function to a variable \texttt{openfile\_id2}.
...
Lets do the easy bit first -- to tell \textbf{MATLAB} to skip \textit{n} lines of a file, you add the parameter \texttt{'HeaderLines'} to the list of parameters passed to \texttt{textscan}, and then simply tell it how many lines to skip. In this Example, you'd add:
\begin{docspec}
my\_data = textscan(openfile\_id2, ... ,'HeaderLines',3);
\end{docspec}
...
\begin{docspec}
my\_data = textscan(openfile\_id2,'\%d \%d','HeaderLines',3);
\end{docspec}
Try it (including closing the file, and a call to \texttt{cell2mat}, as before). What has happened?

It seems that MATLAB translates your format (\texttt{'\%d,\%d'}) into: 'read in a pair of integers, and keep automatically repeating this, until something else is encountered'. That something else is sequence of characters at the end of the first data line (line \#4, because we skipped the first 3), that makes \textbf{MATLAB} think that it has finished (or rather, that it cannot reading in 2 pairs of integers any longer). This leaves you with 2 pairs of integers -- i.e. a 2\(\times\)2 matrix (as you'll see if you look at \texttt{my\_data\_array}).

Here is a solution -- we could omit all the information following the first 2 elements (something for Google to help with).\sidenote{This turns out to be specifying \texttt{'\%*[\(^{\wedge}\)$\backslash$n]'}, which in effects sort of says:\\'skip everything (all the fields) (\texttt{\%*}) up until the end of the line is found (\texttt{[\(^{\wedge}\)$\backslash$n]}).}: 
\begin{docspec}
my\_data = ...
\\ \ \ \ \ \ textscan(openfile\_id2,'\%d \%d \%*[\(^{\wedge}\)$\backslash$n]','Headerlines',3)
\end{docspec}

\item Now close the file:
\begin{docspec}
fclose(openfile\_id);
\end{docspec}

\item And now convert the results to something more human-readable:
\begin{docspec}
my\_data\_array = cell2mat(my\_data);
\end{docspec}
...
\item Use \textbf{MATLAB} function \docenv{xlsread}.
\end{enumerate}
...
\newthought{As an example}, lets return to the paleo proxy CO\(_{2}\) data again, but this time, as an \textbf{Excel} sheet. The data file you need is:
\\\noindent \textbf{paleo\_CO2\_data.xlsx}
...
From the help box on \texttt{xlsread}, it should be pretty apparent what you do. And in fact, I am going to leave you to work it out -- try and import the age and CO\(_{2}\) data (the numeric part of the data) from \textbf{paleo\_CO2\_data.xlsx}.

If you need to, you index a cell array, pretty well much like a normal array, except it has an alternative syntax. For a normal, numeric array \texttt{A}, you might write:
\begin{docspec}
>> A(4,3)
\end{docspec}
...
>> C\{4,3\}
\end{docspec}
and you'd get a value of \texttt{3000} returned in this example.
...
\newthought{For a \textit{netCDF} Example}, we'll take the output of a low resolution Earth system model (\href{http://www.seao2.info/mycgenie.html}{GENIE}). To start off, download the '2D marine sediment results' \textit{netCDF} file -- \textbf{fields\_sedgem\_2d.nc}. The data here is relatively simple -- a 2D distribution of bottom-water and surface sediment properties, saved at a single point in time. In other words, there are only 2 (spatial) dimensions to the data\sidenote{Adding time would make it 3 dimensions (2 spatial + 1 of time). Adding height or depth in the ocean would also make it 3 (3 spatial). 3 spatial + time would make for a 4-dimensional dataset ...}.

OK -- we'll start by opening the file (assuming that you have downloaded it!), remembering to assign its unique ID to some variable.
Then, you'll want to get hold of (and assign to another variable), the ID\ of the variable we want to extract and plot -- in this Example, it is called \textsf{'grid\_topo'}. Having obtained the ID for this variable, you can then fetch it -- assign it to a variable \texttt{data}. Then close the file.\sidenote{You should be able to do all of this without further hints -- the sequence of commands and how they are used, is given in the introduction to this subsection.}

You should now have an array called \texttt{data}. It should be 36\(\times\)36 in size. Why not plot it\sidenote{Your choice of 2D plotting function.}. Can you guess what it might be? Is it in the correct orientation? (If not -- correct it.)
...
The final task would then be to take the 2 axis vectors, and create a pair of matrices -- one containing longitude values associated with the 2D data points, and one containing latitude values  associated with the 2D data points. For this, you would need to use the function \texttt{meshgrid}\sidenote{See subsequent section.}. (We'll re-visit this example once you have seen \texttt{meshgrid} in action.)
...
\newthought{In a related \textit{netCDF} Example}, we'll extend the problem to 3D -- 2 spatial dimensions (longitude and latitude) and one of time. The file you will need to download to experience these wonders, is called \textbf{fields\_biogem\_2d.nc}\sidenote{The back-story is that this contains the 2D surface ocean and atmosphere fields form a model experiment in which the climate system was spun-up from rest and uniform values of everything, so as time progresses, the spatial patterns of the climate system start to evolve and stabilize.}. You are going to go through the same basic procedures of:

\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item Opening the file.
\item Obtaining the ID of the variable (the name of the variable we will use is called \textsf{'atm\_temp'}).
\item Accessing the data using that ID, i.e.:
\begin{docspec}
data = netcdf.getVar(ncid,varid);
\end{docspec}
\item And closing the file.
\end{enumerate}

How many dimensions does this array have?\sidenote{e.g. use \texttt{size}, or ensure that the \textsf{Size} column in the\textsf{ Workspace window} is selected.} What are the lengths along each dimension? Can you guess which dimension of the 3 time might be?

The name of the time axis variable is \textsf{'time'}, and you can extract all the values (different years, in this case) along this axis by:
\begin{docspec}
varid  = netcdf.inqVarID(ncid,'time');
\\times = netcdf.getVar(ncid,varid);
\end{docspec}

Ideally, you should be able, given the 3D array that you have obtained (from the data variable \textsf{atm\_temp}), to specify and plot, the 1st model-projected surface air temperature distribution, as well as the last distribution. And given that the variables for latitude and longitude are also \texttt{'lat'} and \texttt{'lon'}, you should be able to plot the temperature distribution with appropriate axes (and contoured).

Finally, test your understanding to date, by creating an animation of how the surface air temperature in the model evolves over time.\sidenote{You have everything you need -- the \textit{vector} of years, and from this you can determine how many different time points (and 2D data slices) there are, and hence the number of iterations of a \textit{loop}.}
...
\newthought{Back to the 'quake Example}: Find\sidenote{Intentional joke *and* clue.} how may earth quakes there were bigger than M = 8? Also determine how many quakes occurred bigger than M = 7, 6, 5, 4, and 3. Determine the day on which the magnitude 8.7 shock occurred.

In the first problem (number of quakes greater than a specified limit) --  you need ask find to return the row numbers for all quakes satisfying the condition: \texttt{magnitude > 8.0}. \texttt{find} will return you a column vector. You don't actually need to worry about or access the contents of the vector, you just need to know how many elements there are in the vector (because there will be one element for each occurrence of \texttt{magnitude > 8.0}). This is the same as its \texttt{length} (see earlier and/or \textbf{help}).

In the second problem -- you need to find the row number of the quake magnitude data which satisfies the condition: \texttt{magnitude > 8.7}. Knowing the row number, you can then access the data column containing the sate information, and hence extract the day and solve the problem.

All these problems can actually be solved in a single line of \textbf{MATLAB}, but feel free to break it down into multiple steps.
...
\newthought{In the sealevel (oxygen isotope) Example}, you could start by determining the  maximum and minimum sea-levels that have occurred over the last 782,000 years. Then ... because it would be helpful to know *when* the minimum and maximum sea-level heights occurred, use the \texttt{find} function to find the data row in which the minimum and maximum values  occurred. Once you know the respective data rows, you can then easily pull out the ages.\sidenote{HINT -- if your maximum value was stored in the variable max\_value, you found find the corresponding row by:
\\\noindent \texttt{find(data(:,2) == max\_value)}
\\What this is saying, is search the 2nd column (the sea-level values) of the array \texttt{data}, and look for a match to the value of \texttt{max\_value}. The equality operator (\texttt{==}) is used in this context.} 
Find the ages of both minimum and maximum values.

Also find all the occasions (times) on which sealevel was higher than today (modern). (Or equivalently, when the oxygen isotope value, that we are assuming directly reflects changes in level, was lower than modern\sidenote{Lower d18O => less ice volume => higher sealevel.}.)

You can also ask questions based in time, such as what was the sealevel (or oxygen isotope value) at 21 ka (i.e. without having to look through the data manually and determine on which row 21 ka occurs, because this is exactly what \texttt{find} can do this for you)? This can be particularly useful if the value of time is calculated or passed in from elsewhere, rather than specified as e.g. 21 ka, because you may not \textit{a priori} know what the value will be, hence automating the script with \texttt{find} is super useful. Effectively then you are creating an \textit{algorithm} for taking a time input and determining sealevel.
...
\newthought{For an Example of data-filtering} -- dig out the paleo-proxy (not ice-core) atmospheric CO\(_{2}\) data you downloaded. One further way of plotting with \texttt{scatter} is to scale the point size by a data value. We could do with by:
\begin{docspec}
...
The command \docenv{set} can be used in the context of any (I think?) graphics object, i.e. component part of a final plot (e.g. axis, ticks, line, point, etc). For example, in creating the line segment previously:
\begin{docspec}
h = line(X2,Y2);
\end{docspec}
...
If you have the \textbf{MATLAB} \textsf{Image Processing Toolbox}, then you can use the command viscircles to draw circles.
...
You are already familiar with setting colors for lines, with the notation: \texttt{'r'}, or \texttt{'b'} (for red, blue, respectively). This is nice and simple and so totally fabulous ... except there are a limited number of colors available in this notation:
...
There is not much to placing text and specifying its properties. The \textbf{MATLAB} command for writing a string to a \textsf{figure window}, is \docenvdef{text}.
...
This (algorithms (e.g. see Box) and problem-solving) is not something that can really be taught. So we'll go through a series of step-by-step examples. Hopefully this will illustrate some general coding approaches.

%------------------------------------------------

\subsection{Example \#1: max(!)}

So yes, this is a built-in \textit{function} \texttt{max} in \textbf{MATLAB}, but suspend disbelief, and pretend that there is not one. What if we wanted to create one, i.e. \textit{function} that is passed a vector of numbers, and returns the maximum value?

So already, from the definition of the problem, you know to create a new \textsf{m-file} a define a \textit{function}, that takes as input a variable (a vector), and returns the largest value in that vector.\sidenote{Note that you do not need to specify that the input variable is a vector, just that there is a variable input.}\sidenote{Here, my personal naming convention has:\\
...
You are still not solving the problem, but in this step, you have now demonstrated that your functions can take a (vector) input, do something with it, and set the output variable.
...
MATLAB is unhappy about the line where the value of \texttt{temp\_max} is being tested against v\_in(n). It may be obvious to you when (in terms of loop iteations) this is occurring. If not, why not, just after:
...
\newthought{If you are passably happy with that --} try writing a (new) function that finds the \textit{minimum} element in the array.\sidenote[][-1.0in]{This is in fact employed a little later, where it is referred to as \texttt{minxx}.}
...
\vspace{-2mm}
\newthought{In this second Example --} you have a vector of numbers, and you wish to sort them into ascending order. The function would take a vector as input, and return a vector of the same length as output, comprising all the value of the input vector, but sorted.
...
This will work, but only return the lowest (1st) value, rather than sorting them all. You might spot that the 3rd through 6th lines of code, need to be repeated -- i.e. you keep calling the find-the-minimum function, and adding this value to the output vector. But if this is all you did, the program would run forever and return an infinite number of repeats of the minimum value -- this is because once you have found the minimum value, and added it to the output array, the value still exists in the input array and will be found again (and again and again) if you create a loop.

The key, at least, the key to creating the particular algorithm/solution I will outline next, is to remove the element from the input array, that you have just used. e.g. if we find a '\texttt{1}' and this is the lowest value, after it has been added t the output array, it needs to be deleted from the input array. This means that we need to know 'where' in the (input) array the minimum value was, so you next task is to \uline{modify your \texttt{minxx} function, to return the position in the vector that the minimum value occurred at} (in addition to the value of the minimum value itself).\sidenote{HINT: When you find a new minimum value in the loop, the index (the position of that minimum value) is \texttt{n}. So you need to create a temporary variable that you update with the value of \texttt{n}, only when a new minimum value is found in the loop. You will see this variable with '\texttt{1}', consistent with assigning the first minimum value from index \texttt{1} in the vector.} i.e. your \texttt{minxx} (or whatever you called it) function needs to look like:
\begin{docspec}
\textcolor{blue}{function} [s\_out1 s\_out2] = minxx(v\_in)
\end{docspec}

Having obtained the index of the minimum value, you can delete thsi entry from the input array by:
\begin{docspec}
v\_in(m) = [];
\end{docspec}
where \texttt{m} is the index.

The complete code within your sort function, minus your (modified) \texttt{minxx} function, might look like\sidenote{Note the notation for catching the 2 returned values from the \texttt{minxx} function:
...
\begin{docspecnormal}
\(F_{in} = \frac{(1-\alpha) \cdot S_{0}}{4}\)
\end{docspecnormal}
where the present-day value of the solar constant, \(S_{0}\) is \( 1368 Wm^{-2}\)
...
\\Albedo (\(\alpha\)) -- the fraction of incoming solar radiation that is reflected back to (-wards) space -- varies hugely across surface types (and angle of incoming radiation). A commonly used mean global approximation is to set: \(\alpha=0.3\).
...
The exercises that follow are structured and you need to pay attention to which \textsf{m-files} you are creating from scratch, which ones, having been created and coded up, you  do not then further edit ... 

\begin{description}[align=right]

\setlength{\itemindent}{-0.2in}

\item [7.1.1] In this first Subsection ('\textit{The basic EBM}'), you'll  create a script (\# \texttt{scr\_1\sidenote{This is not a suggested name of the \textbf{m-file}, but an ID to help you not get confused as to which script or function is being referred to in the text ...}}) containing the Energy Balance Model (EBM), and test it.

\noindent (See Figure \ref{fig:ch8-schematic-scr1}.)

\item [7.1.2] Next, you'll turn your EBM script (\texttt{scr\_1}) into a function (\texttt{fun\_1})\sidenote{\textbf{Once the EBM function has been created, you do not at any point edit it any further!}} -- passing in the solar constant and albedo as parameters, and returning the surface temperature. (And test it.)

\noindent (See Figure \ref{fig:ch8-schematic-fun1}.)

\item [7.1.3] In the Subsection '\textit{Parameter sensitivity experiments using the EBM -- \#1}', you will create a new script (\texttt{scr\_2}) with a single loop in it. Within the loop, you will make a call to the EBM function (\# \texttt{fun\_1}) that you created.\sidenote{DO\ NOT put code the loops into the EBM function -- leave the function alone ...}

\noindent (See Figure \ref{fig:ch8-schematic-scr2fun1}.)

\item [7.1.3] Then, in an extension to the previous Subsection work, you will create another new script (\texttt{scr\_3}), this time  with a double (nested) loop in it. As before -- within the loop, you will make a call to the EBM function.
...
\item [7.1.4] In the penultimate Subsection ('Calculating the evolution of the solar constant'), you'll create a new function (\texttt{fun\_2}), which will take time (counted from the formation of the Sun) in \(Ga\), and return the value of the solar constant at that time (\(S(t)\) (\(Wm^{-2}\)) ).

\noindent (See Figure \ref{fig:ch8-schematic-fun2}.)

And then ...

\item [7.1.5] ... finally (Subsection '\textit{Evolution} of Earth's surface temperature'), you'll create one last script (\texttt{scr\_4}), with a loop in time in it, and from within this loop, you'll call first the solar constant function (\texttt{fun\_2}), taking time as an input and returning the value of \(S(t)\), which you will then pass into the EBM (\# \texttt{fun\_1}), returning \(T\).

\noindent (See Figure \ref{fig:ch8-schematic-scr4fun1fun2}.)

\end{description}

Note that because you will be creating, running, and no doubt debugging a number of different scripts, that it is beneficial to add the line:
\begin{docspec}
clear all;
\end{docspec}
somewhere near the very top of the \textsf{m-file} (but before you try and define any parameters). This is because in script files, all the variables are global and appear in the MATLAB workspace. When you run a script a 2nd time (or a different script), it is possible that pre-existing variables or arrays might interfere with your script, or at least create the potential for confusion.\sidenote[][-3.25in]{This is different from \textit{functions}, where all the variables within the \textit{function} are \textit{private} and hence isolated from the rest of the \textbf{MATLAB} universe.}
...
To kick off -- create a new script (\textbf{m-file}) ('\texttt{scr\_1}' in the summary notation) and code up the analytical solution to the basic global mean energy budget at the surface of the Earth (see Box) in a program structure illustrated schematically in Figure \ref{fig:ch8-schematic-scr1}.\sidenote[][0.0in]{
...
For now -- prescribe the value of \(S_{0}\) -- for which the modern value is 1368 Wm\(^{-2}\) as well as the value of surface albedo (\(\alpha=0.3\)  by default) -- somewhere near the start of the program. Then run it.
...
\item If still 'no' -- maybe take the 2 component equations (for \(F_{in}\) and \(F_{out}\)), plug \(S_{0}\) into the equation for \(F_{in}\) and then play with different values of \(T\) to find a value for \(F_{out}\) that is approximately equal -- is the value for \(T\) sane? If not, double-check the units and values in both component equations.
\item If still 'no' -- WHAT HAVE YOU DONE?
\end{enumerate} 

Once it is working, have a quick play about, changing the value of \(S_{0}\) and albedo (\(\alpha\)) (saving the \textsf{m-file} each time and re-running) to get a vague feel for how sensitive the surface temperature is to these two parameters.
...
\end{docspecmargin}
} that takes in 2 parameters -- the solar constant (\(S_{0}\)) and the mean global albedo (\(\alpha\)). The function should return the global mean surface temperature, \(T\).\sidenote{Note that the parameters passed into, and returned by, the function, can be called anything you want. As long as they are useful (and clearly defined/explained in a comment somewhere).} (See Figure \ref{fig:ch8-schematic-fun1})

Try playing with the function in the same way as before, but now passing the different values of \(S_{0}\) and \(\alpha\) (rather than having to edit the \textsf{m-file}, save, and re-run each time). To use the function (assuming you called it e.g. \texttt{fun\_1}), and assuming the 2 passed parameters are in the order: \(S_{0}\), \(\alpha\) and are given their default values, you'd write (at the command line):
\begin{docspec}
>> fun\_1(1368.0,0.3)
\end{docspec}
(and get a value close to 14\degree C returned).

%------------------------------------------------

\subsection{Parameter sensitivity experiments using the EBM -- \#1}

Now to utilize your new function ('\texttt{fun\_1}' in the summary notation). Create a new blank script ('\texttt{scr\_2}') and define 2 parameters near the start -- one for the value of \(S_{0}\) and one for \(\alpha\), then further down the code, call your function (\texttt{fun\_1}), passing it these 2 parameters \uline{but remembering} that you need to assign the result of your function to some \textit{variable}\sidenote{i.e.
\begin{docspecmargin}
T = fun\_1(1368.0,0.3);
...
Common in numerical modelling is quantifying how sensitive a system is to the choice of parameter values -- called a \textit{sensitivity experiment}. You may already have gotten a feel for roughly how sensitive \(T\) was to changing \(S_{0}\) on its own, or changing \(\alpha\) on its own, but what about when both parameters vary together?

Lets start with a simple 1-D case, and consider just a change in the value of \(S_{0}\). To automate generate different values of \(S_{0}\) and call the function, you are going to need a loop\sidenote{You are going to put the loop in the script (\# \texttt{scr\_2}), NOT the function (\# \texttt{fun\_1}).
...
\item [\textbf{loop option \#1}] You could loop directly through the range of values of \(S_{0}\) that you are interested in, e.g.
\begin{docspec}
\textcolor{blue}{for} S0 = 1000:100:1500
\\ \ \ \ \ \textcolor[rgb]{0,0.501961,0}{\% CODE GOES HERE}
\\\textcolor{blue}{end}
\end{docspec}
in which \(S_{0}\) will go from 1000 to 1500 Wm\(^{-2}\) in steps of 100 Wm\(^{-2}\) \sidenote{You can pick a different range and increment ... this is just a quasi-random example to illustrate ...}.
...
\\\textcolor{blue}{end}
\end{docspec}
which now passes exactly through the modern value of \(S_{0}\).
...
\\\textcolor{blue}{end}
\end{docspec}
Look carefully through this code and follow what is going -- as \texttt{m} counts from \texttt{-5} to \texttt{5} (in steps of\texttt{ 1}), \texttt{100} times the value of \texttt{m} is added to the modern value of \(S_{0}\)\sidenote{The variable definition \texttt{S0\_modern = 1368.0} at the top of the code fragment.}, meaning that \(S_{0}\) ends up going from \texttt{S0\_modern} - 500, to \texttt{S0\_modern} + 500 Wm\(^{-2}\) (in steps of 100 Wm\(^{-2}\)).
...
And why might we want some sort of an integer counter in the first place? Well, you might want to save the data(!), i.e. the calculated (by your function) value of \(T\) vs. the inputted value of \(S_{0}\). This data will need to go into an array, with one row corresponding to each value of \(S_{0}\).
...
data\_S0 = zeros(11,1);
\end{docspec}
which would create a (single) column vector with 11 rows. You'd need an equivalent vector (e.g. \texttt{data\_S0} in this example) for storing the corresponding value of \(S_{0}\) used in the temperature calculation. These vectors are created before the loop starts.
\\Then within the loop (and after the calculation of \(T\)), you'd assign your values of \(S_{0}\) and \(T\) by using whichever index you created\sidenote{i.e. which of the two OPTIONS you chose earlier.}:
\begin{docspec}
\ \ \ data\_S0(m) = S\_0;
\\ \ \ \ data\_T(m) \ = T;
\end{docspec}
...
\item [\textbf{save option \#3}] Or ... \textbf{MATLAB} will allow you to 'grow' a vector, one element at a time (but not for matrices).\sidenote[][-2.0in]{
...
Finally, at the end of your program (after the end of the loop), you can now plot (\texttt{plot} or \texttt{scatter}) how \(T\) varies as a function of \(S_{0}\), having saved all the values of \(S_{0}\) you tested, plus the corresponding calculated temperatures, in a handy matrix (or pair of vectors). Note that regardless of whether you use \texttt{plot} or \texttt{scatter}), you need to specify to \textbf{MATLAB} that you want the values in the 2nd column of the array (\textit{y}-axis), plotted against the first column (\textit{x}-axis).\sidenote[][-4.0in]{Refer back to the earlier chapters on plotting of e.g. the CO2 or glboal temperature data you analysed, and also recall how to specify e.g. all the rows in the first column of an array.}

The structure of your code should look like Figure \ref{fig:ch8-schematic-scr2fun1}. and your resulting figure (depending on the range you assume for \(S_{0}\)), something like Figure \ref{fig:ch8-sens1D}.
...
In this Sub-sub-section, we'll extend the parameter sensitivity analysis of your model to 2D, assuming for instance that you are  now interested in how \(T\) also varies as a function of \(\alpha\) (surface albedo). So, you'll need to vary both \(S_{0}\) and \(\alpha\), and in all combinations of the two. In fact, in a grid pattern, with \(S_{0}\) increasing in steps on one axis (as before), and \(\alpha\) on the other.

Hopefully, you might have guessed that you'll need a \textit{nested loop}(?) -- one loop going through all possible values of \(\alpha\), \textit{for each and every possible value of \(S_{0}\)}??

Perhaps, as an aside, we'll go through a simpler example/system first.

A chess board consists of squares in a \(8\times8\) grid. The squares alternate black and white. To define 8 squares (points) along the \textit{x}-axis on the bottom row, you'd write something of the form:
\begin{docspec}
...
OK -- that was easily the greatest and least interesting diversion in pedagogical history, but nested loops should now come almost as second nature to you :o) So how about coding up the nested loop for the question we were meant to be addressing -- carrying out a 2D sensitivity test of the parameters \(S_{0}\) and \(\alpha\). See if you can create this.

Start with a new (script) \textbf{m-file} ('\texttt{scr\_3}'). For constructing the loop -- you have already seen the 1D example of parameter sensitivity code, and also an example of creating a nested loop for a 2D grid. Your chess board columns (\texttt{m}) become \(S_{0}\), and rows (\texttt{n}) become \(\alpha\). You don't need to do anything so awful as that \texttt{if \ldots} statement -- instead just call your function (\texttt{fun\_1}) for solving the global surface temperature (passing it the values of \(S_{0}\) and \(\alpha\) generated in the loop). A schematic of the program structure is shown in Figure \ref{fig:ch8-schematic-scr3fun1}.
...
\ \ \ \ \ \ data\_output(n,m) = T;
\end{docspec}
Don't forget that you'll also need to know the values of \(S_{0}\) and \(\alpha\) that correspond to the column and row numbers. Perhaps save these as 2 individual vector (as per before) or ignore them for now.
...
In this  Subsubsection, you are going to leave the 2D-ness aside and consider how Earths surface temperature may have changed through geological time.

So far you only have a function equating solar constant (\(S_{0}\)) to temperature (\(T\)). What you need is some way of equating time (\(t\)) to the value of the solar constant at that time \(S_{0(t)}\) (which you can then turn into temperature). We'll remedy this toot sweet.

Start by creating a new  (blank) \textsf{m-file} and define it as a \textit{function} that takes in a variable for time, \(t\) (in units of \(Ga\)) and spits out  (aka, returns) the calculated value of \(S_{0(t)}\) (\(Wm^{-2}\)) (this will be '\texttt{fun\_2}' in the on-going notation).

The background to the equation that will go into your function is given in the Solar constant Box. In this, you'll first need to substitute the modern value of the solar constant (\(S_{0(t=0)}\)) into the equation to leave it in terms of \(S_{0(t)}\) (the solar constant value at time \(t\)).
 
Your function, aside from the all-important 1st line (and \textcolor{blue}{\texttt{end}} at the end) and appropriate \textcolor[rgb]{0,0.501961,0.25098}{\texttt{\% comments}}, need have little more in than a definition for any constant you might want to use, such as the modern value of \(S_{0(t=0)}\) and perhaps the reference time\sidenote[][2.0in]{Which is also equal to the current time (since the formation of the Sun).} (\(t_{0}\)) (4.57 Ga) ... and a single line for the equation giving the value of \(S_{0(t)}\). 
...
\\The value of \(L_{0}\) is equivalent to a flux (\(Wm^{-2}\)) of 1368 Wm\(^{-2}\) incident at the top of the atmosphere at Earth. In the equation, \(L\) can hence be substituted for \(S\) to give the value of \(S_{0}\) (\(Wm^{-2}\)) at any time, i.e. \(S_{0(t)}\), i.e.
\begin{docspeclarge}
\(\frac{S_{0(t)}}{S_{0(t=0)}} = \frac{1}{1+\frac{2}{5}\cdot (1-\frac{t}{t_{0}})}\)
\end{docspeclarge}
or, in terms of the value of \(S_{0}\) at time \(t\):
\begin{docspeclarge}
\(S_{0(t)} = \frac{S_{0(t=0)}}{1+\frac{2}{5}\cdot (1-\frac{t}{t_{0}})}\)
\end{docspeclarge}
...
\\The reference value of \(S_{0}\): \(S_{0(t=0)}=1368\) Wm\(^{-2}\).
\end{mdframed}}
...
Finally ... you are going to bring it all together and calculate and then plot  the surface temperature of the Earth, at 100 Myr intervals, from 4.0 Gyr (4 billion years) in the past, to 4.0 Gry in the future -- spanning approximately the age of the Earth and much of its potential long-term future. (Remember, if you did not attempt the previous mini exercise to convert the function for \(S_{0(t)}\) so that it worked with time relative to present-day, you are going to need to convert the range, of \(-4 \rightarrow\ +4\) Gyr relative to present, to time relative to the formation of the Sun.) 

Start by creating one final new (yet another blank \textsf{m-file}) script ('\texttt{scr\_4}').. You are going to need a loop in time, perhaps looping from 4.0 to -4.0 Ga relative to now (but you can chose what limits you like ... except remembering the Sun is only 4.57 Ga old ...). Within the loop, you will:

\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item Pass to your solar constant function your time variable, and obtain the corresponding value of  the \(S_{0(t)}\).
\item  Call your zeroD EBM function to calculate the corresponding surface temperature, passing it the value of \(S_{0(t)}\) that you have just calculated.
\item Store in an array, or pairs of vectors, time and the corresponding value of \(T\).
\end{enumerate}

Likely bug possibilities include the units of time (Gyr), and that time in the equation for \(S_{0(t)}\) is counted forwards from the formation of the Sun. Also be careful with nested parentheses (()). A schematic of the program structure is shown in Figure \ref{fig:ch8-schematic-scr4fun1fun2}.
...
Assuming that you have managed something like Figure \ref{fig:ch8-ebm-solarconst}\sidenote[][0.25in]{Note that a line has been added to highlight \(t=0\) (i.e. the present-day) -- see \docenv{line} (see earlier).} -- what strikes you, in light of (hopefully) what you know about the past history of climate and evolution of life on this planet, about your model projection (for the past)? What is 'missing'?
...
i.e. the rate of change of some variable \(x\), is some function of itself (\(x\)).\sidenote{The equations need not be a function of time.}
...
\noindent where \(O(\Delta t^{4})\) represents 4th order (and smaller) terms (or an 'error', if not accounted for), that will be smaller in magnitude than \(\frac{\Delta t^{3}}{6}\cdot\frac{\partial x^{3}}{\partial t^{3}}\).
...
Consider the radioactive decay of an amount of radioactive substance. Assume an initial activity \(A\) (don't worry about what the units of this activity are), and the substance decays such that after 1 day, the activity is half the original activity. We could write (or you might see given to you):
...
This simply says: the rate of change in \(A\) with time (days), is equal to minus (because it is decaying rather than growing) its value.
...
\marginnote[-0.75in]{\begin{mdframed}[backgroundcolor=light-blue, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
The system we'll consider is shown schematically in Figure \ref{fig:ch9-ball}. In essence: we want to determine \(d\) -- the horizontal distance (\(m\)) that the ball travels before it hits the ground. The initial conditions are:
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item The ball is thrown from an initial height \(h\) (\(m\)).
\item The ball is thrown with an initial speed \(s_{0}\) (\(ms^{-1}\)).
\item The ball is thrown at an initial angle \(\theta\) with the horizontal.
\end{enumerate}
...
Now to add some code to the loop. In each time-step, i.e. each time around the loop, \(dt\) time (\(s\)) passes. In time \(dt\), if the horizontal velocity of the ball is \(u\), you should be able to calculate how far it moves, right?\sidenote{Distance = velocity times time: \\\(dp=u\times dt\)} You need to add this increment in distance travelled to the current value of the position variable \(p\)\sidenote{i.e. with code like 
\begin{docspecmargin}
p = p + delta\_p;
...
\newthought{\textit{\textbf{Part III }}} You should now have 2 working models (sperate \textbf{m-files}) -- one for the horizontal position of the ball, and one for the vertical position (and vertical velocity) of the ball. You now want to combine the 2 sperate parts of the model.\sidenote{I suggest basing the combined model on the vertical model (as it is the more complicated of the 2) and hence copying-and-renaming the 2nd script.}
...
By now, you should have noted a slight problem -- in both previous (sperate) models, the variable \(h\) was used to represent both horizontal AND vertical velocity. \docenvdef{D'uh!}\marginnote{
\docenvdef{duh}
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item A conditional to test whether the vertical position has dropped below zero. This would go in the loop just after the position of the ball has been updated, And ...
\item The \textbf{MATLAB} command to exit a loop, which you have seen before.
\end{enumerate}
...
Open a new graphics figure window, before the loop starts and set \texttt{hold} on by adding the lines
\begin{docspec}
figure;
\\hold on;
\end{docspec}
...
\sidenote[][-1.0in]{
A slight complication here as that as it stands, the code will not work because in the first time around the loop, when you get to \texttt{delete(h)}, \textbf{MATLAB} is unhappy becasue the \textit{handle} \texttt{h} has not yet been defined anywhere.
...
\chapter{Graphical User Interfaces (GUIs)}
\label{ch:09}
...
\textbf{MATLAB} kindly\sidenote{For once, it is not a sperate, zillion-dollar license ...} provides a tool (itself a GUI) for creating GUIs -- the 'Graphical User Interface Development Environment' (\textbf{GUIDE}). \textbf{GUIDE} does 2 main things for you:
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item Firstly, it facilitates the design of the GUI window(s).
...
Before you move on, it is worth pausing at this point and reflecting on what happened and what the implications are for what you might like to do (GUI-wise). At the command line, you entered the command \texttt{guide}, which presumably ran a script or function (a piece of code in any case). A window (the '\textsf{GUIDE Quick Start}' window) was summoned (actually a \textsf{figure window} was created). The (figure) window did not open completely blank, but instead you might note it has:
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
...
When you are done (editing properties) -- click the \textsf{Save} icon. If this is a GUI that you have not previously created or previously assigned a filename to, you'll get a \textsf{Save As} dialogue box. At this point, \textbf{MATLAB} is going to save the window design with a \textsf{.fig} extension.
...
Close the design window (and the code editor if it distracts you). At the command line, type the filename (no extension) to run the automatically generated code \textsf{m-file}. A window opens up ... the contents should come as no surprise, because you have just specified them (via the GUIDE GUI design tool). Your first GUI! But one you might notice does not actually do anything -- it just sits there unresponsive. Although you can at least close it (because it is automatically generated with the usual basic close/minimize icons plus the name of the m-file in the \textit{titlebar}.

%------------------------------------------------

\subsection{Simple GUI responses [\textit{Push Button}]}

A GUI is only of any particular use if it allows some response to input. This is going to involve a little code of your own ... so we'll start with the simplest possible action -- a \textit{button} that performs a simple action (closes the window).

Re-run the \texttt{guide} program and open up a new window editor (by clicking \textsf{OK} in the \textsf{GUIDE Quick Start} window). Now find the \textsf{Push Button} icon, click it, and drag out a push button object in the design pane. You should see a box (with a pseudo 3D shading at the edges) with the text \textsf{Push Button} in the centre as per Figure \ref{fig:chGUI-PushButton}. As before, you can edit the properties of the push button object (because the default properties are totally boring) by double-clicking it. Start by editing the font (size) and message. Perhaps 'Go away!'. And then save it.
...
When it saves, \textbf{MATLAB} again opens up the code associated with the figure window that it has automatically generated. There is slightly more code in the file this time and shortly, you'll need to look at it. But for now: ignore it again and type the name of your \textsf{m-file} file at the command line. Again, you'll get a window opening with the push button you created in it. Click on it. It does seems to 'respond' (pretends to depress by means of changing the edges with the pseudo 3-D shading) to the mouse click, but ... nothing else happens. This is where YOU (and your amazing coding skills) now come in.

If you have closed the design window, re-run \texttt{guide} and rather than creating a new GUI -- switch to the \textsf{Open Existing GUI} tab and double-click your filename (of the push button GUI) or select and OK. Double-click on the push button object to open up the property editor. We'll make only one (more) change here -- down the list of properties your fine '\textsf{Tag}'. This is the name (ID or \textit{handle}) of the push button object.\sidenote{In essence, no different from a filename -- a unique identifier for an object (/file).} By default, the name is \textsf{pushbutton1}. Edit this to ... \textsf{goawayButton} (or pick an alternative name) and re-save the GUI.
...
Note that \textbf{MATLAB} does not formally end any of the functions with \textcolor{blue}{end}. Don't get confused as to where to place code -- assume that when the next function starts, the next \textcolor{blue}{function} definition starts, that the previous function has ended. If ti helps -- add in lines (with \textcolor{blue}{end}) to end each \textit{function}. Or perhaps add some ASCII art/comment line before each new function to help make it clearer, e.g.

\begin{docspec}
\textcolor[rgb]{0,0.501961,0}{\% === END FUNCTION ============================================}
\end{docspec}

In this simple GUI, we have only one figure and it is \textit{active} (has the mouses' attention)\sidenote{Often in operating systems -- the active window, i.e. the one that is the one to respond to mouse clicks or key presses, has its titlebar highlighted in a bright color (while inactive ones might be grey.)}, so we could simply use the \texttt{close} command ('\textit{deletes the current figure}') on its own (just this on one line). 

Insert this simple command in the
\begin{docspec}
\textcolor{blue}{function} goawayButton\_Callback
\end{docspec}
 \textit{function}, after the last comment line.\sidenote{Note that automatically generated \textbf{MATLAB} code does not seem to ever formally \texttt{\textcolor{blue}{end}} a function as one really should do ...} Save the \textsf{m-file} and re-run. Now if you run your program and click on the 'Go Away!' push button, the window does indeed go away (aka, closes).
...
Bananas. Do you like them? Perhaps the GUI can provide an answer (rather than just text statements written  to the command line via \texttt{disp} as before).

Now you are going to want to think about the design of the GUI a little. Perhaps sketch out a sign on paper\sidenote{The white flat stuff that you write on. Maybe you have forgotten what it is? Clue: it is not an app on iTunes.} first.
...
Firstly -- re-run GUIDE. Create a new GUI window with the 4 elements (2 static text boxes and 2 push buttons). It is up to you how you arrange these 4 objects in the design pane. You might be guided how windows in programs that you have used, are designed. At the minimum, it is standard practice to place a 'No' push button next to and aligned horizontally with, the 'Yes' (and often 'Yes' to the right of 'No').
...
The code that \textbf{MATLAB} generates for \textsf{bananas.m} (my name choice!) is not a lot more involved than before. Primarily, there is just a second function associated with a mouse click on the 2nd push button. So the end of the automatically generated \textbf{MATLAB} code now looks like:
...
Unfortunately, \textbf{MATLAB} does get all weird here.\sidenote{Actually, no weirder than \textbf{netCDF}. Or arguably, \textbf{Python} ...} If you had a friend called Luna, you might reasonably communicate with them via the name 'Luna'. \textbf{MATLAB} doesn't do it this way and instead assigns a numeric ID. Think of it as rather than storing information in a database about Luna and by name, information might be stored by SSN instead. So too retrieve or write information about Luna, you do it via their SSN.\sidenote{Or you could think about university student databases and access via the unique student number.} Here, we want to change a property (the displayed text  of the 2nd text box), and you are going to have to get its ID first in order to do it.

First off, you can get the ID of the object property using \texttt{findobj} and assign the result to some memorably variable\sidenote{Here, the \texttt{h} bit stands for 'handle' but you might chose \texttt{id} for ID instead?.}, e.g.
\begin{docspec}
...
\noindent To put your coding skills more to the test: how about displaying a 3rd message ('Make up your mind!'?) if someone changes their mind -- i.e. if a second button is pressed (after the first one). You'll need a variable to store whether any button has been pressed and assign this an initial value of \textit{false}, e.g.
\begin{docspec}
var\_pressed = false;
\end{docspec}
...
\end{docspec}
This line needs to appear at the start of each \textit{function} in which you need to read or write the value of \texttt{var\_pressed}, i.e. in both callback \textit{functions} as well as the initialization \textit{function}.
...
\end{docspec}
also appearing in \texttt{\textcolor{blue}{function} bananas\_OpeningFcn}.
...
\begin{enumerate}
\setlength{\itemindent}{.2in}
\item First create a new (blank) GUI.
...
This is no so different form the button example -- when the slider bar is dragged, or the up/down arrows are clicked (and the slider bar moved that way), this \textit{function} is called. It is then up to you to:
...
To set the text in the \textsf{Static Text} box object, as befor eyou need to obtain the ID of the object:
\begin{docspec}
...
Now ... if the value of \texttt{bananaindex} goes above 5, make the text box background blue. And it \texttt{bananaindex} is below (or equal to) 5, set the color to red.
...
\newthought{But before anything ...} -- we'll go through a few of the graphics procedures you will need to grasp and implement.
...
axes('pos',[x,y,dx,dy])
\end{docspec}
where (\texttt{x},y) are the coorindates in the window, which by default are from \(0-1\) in both \textit{x} and \textit{y} directions. \texttt{dx} and \texttt{dy} are the width and height, respectively, of the axes (in the same window coordinate system).
...
\\Whereas, within the loop, you are defining a small axes region to contain the image. (And whose location and width/height are given in relative (\(0-1\) scale) units, rather than \(0-x_{max}\) and \(0-y_{max}\).)
