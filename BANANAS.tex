...
A visual guide to the structure of your programs is given by schematic figures in the page margin\sidenote[][0.0in]{Not all code fragments and programs are given a schematic.}. For example, a generic \textit{script} (yellow box) is shown by \textbf{Figure \ref{fig:ch0-script}}, and a generic \textit{function} (green box) by \textbf{Figure \ref{fig:ch0-function}}. \sidenote[][0.0in]{Don't worry about the terms \textit{function} and \textit{script} for now ....}
...
Or if you prefer -- create a new (empty) \textit{function} (select \textsf{New} and then \textsf{Function} from the \textbf{MATLAB} toolbar/menu-bar) and then copy-paste in the contents of \textsf{scr\_1.m}. Note that in the \textit{function}, you no longer define yourself the values for \texttt{solar\_constant} and \texttt{albedo} in the \textbf{m-file} -- instead, these values are passed into the \textit{function} when you call it. For instance, if at the top of the \textit{function} (see side-note) you  defined:
\begin{docspec}
...
\marginnote[-2.5in]{\begin{mdframed}[backgroundcolor=light-blue, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
Likely bug possibilities include mistakes with nested parentheses \texttt{(())}, units (e.g. \(K\) vs. \(\degree C\)), and time, which for now should run forward from zero (the formation of the Sun). A schematic of the program structure is shown in Figure \ref{fig:ch8-schematic-scr4fun1fun2} to aid you.

Assuming that you have managed something like Figure \ref{fig:ch8-ebm-solarconst}\sidenote[][-0.0in]{Note that a line has been added to highlight \(t=0\) (i.e. the present-day) -- see \docenv{line} (see earlier). This plot also has an altered time-axis and time is plotted relative to now -- see below.} -- what strikes you, in light of (hopefully) what you know about the past history of climate and evolution of life on this planet, about your model projection (for the past)? What is 'missing'?
...
\begin{docspecmargin}
temp = fun\_1(S,albedo);
\end{docspecmargin}assigns the result of your temperature calculation to the variable \texttt{temp}.}. So far so boring, as this is in effect what you had been doing previously in 'playing' with the energy balance function. 


Starting with a simple 1-D case and considering just a progressive change in the value of \(S\), to automatically generate a progression of different values of \(S\) and call the \textit{function}, you are going to need to create a loop\sidenote{You are going to put the loop in the script (\# \texttt{scr\_2}), NOT the function (\# \texttt{fun\_1}).
...
\ \ \ data = [data; S temp];
\end{docspec}
i.e. concatenating a vector \texttt{[S temp]}  (and hence a complete row) to the end of the matrix \textbf{data}.
\end{enumerate}
...
Finally, at the end of your program (after the end of the loop), you can now plot (\texttt{plot} or \texttt{scatter}) how \(T\) varies as a function of \(S_{0}\), having saved all the values of \(S\) you tested, plus the corresponding calculated temperatures, in a handy matrix (or pair of vectors). Note that regardless of whether you use \texttt{plot} or \texttt{scatter}, you need to specify to \textbf{MATLAB} that you want the values in the 2nd column of the array (\textit{y}-axis), plotted against the first column (\textit{x}-axis).\sidenote[][-0.0in]{Refer back to the earlier chapters on plotting of e.g. the CO2 or global temperature data you analysed, and also recall how to specify e.g. all the rows in the first column of an array.}
...
\item [6.2.3] Finally (almost) -- we'll allow the daisies affect their *local * (temperature) environment. Now it gets more interesting (honest!). Although the code structure is exactly the same as in the last step\sidenote{A loop through geological time, as per in the previous Section. Within this main loop, you'll have a sub-loop with just the daisy function followed by the EBM function.}, you will require a further copy-rename-and-edit of the previous daisy function ('\texttt{fun\_4}' \(\rightarrow\) '\texttt{fun\_5}') and one further copy-rename-and-edit of the previous script ('\texttt{scr\_6}' \(\rightarrow\) '\texttt{scr\_7}') that calls the daisy function.
...
\begin{docspecbold}
\textcolor[rgb]{0,0.501961,0}{\% calculate fractional area of bare ground\\}
Fg = (1.0 - Fw - Fb);\\
\textcolor[rgb]{0,0.501961,0}{\% calculate mean albedo\\}
...
\(\alpha, F_{w}, F_{b}\)
\end{docspec}
(see help of various sorts on \textit{functions}, but it not at all a fundamental change as to compared to before). Of course, you need to substitute the maths symbols for the actual variable and parameter names you choose to use. So ... e.g.
\begin{docspec}
function [albedo, Fw, Fb] = fun\_4(temp, fw, fb)
\end{docspec}
which defines a \textit{function} with 3 inputs, and 3 outputs.
...
\marginnote[-3.0in]{\begin{mdframed}[backgroundcolor=light-blue, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
\marginnote[-1.0in]{\begin{mdframed}[backgroundcolor=light-blue, roundcorner=10pt,leftmargin=1, rightmargin=1, innerleftmargin=5, innerrightmargin=5, innertopmargin=5,innerbottommargin=5, outerlinewidth=1, linecolor=light-gray]
...
Lastly, you need to know the horizontal component of the balls velocity.\sidenote[][-3.0in]{In the absence of air resistance, horizontal velocity does not actually change throughout the simulation (i.e. in each iteration of the loop, it will have the same value).}  You can calculate the (initial) horizontal component of velocity from the given initial conditions of initial speed (\(s_{0})\) and initial angle of trajectory (\(\phi\))\sidenote[][-2.5in]{Just as long as you can remember how to calculate the sides of a right angled triangle given the length of the hypotenuse, which here is the speed ... \\REMEMBER that \textbf{MATLAB} uses \textit{radians} for calculating with angles, not \textit{degrees}.}. For now, pick any 'reasonable' values for \(s_{0}\) \sidenote[][-1.5in]{On September 24, 2010, against the San Diego Padres, Chapman was clocked at 105.1 mph (169.1 km/h) -- the fastest pitch ever recorded in Major League Baseball. If you convert 169.1 km/h into units of \(ms^{-1}\), this will give you some reasonable upper limit for your initial  thrown velocity.} and \(\phi\) \sidenote[][-0.25in]{Obviously, the angle should lie between zero and 90 \degree  (or else the throw is going backwards and/or into the ground). BE CAREFUL as \textbf{MATLAB} assumes that angles are in units of \uline{radians}, so either work in units of radians throughout, or convert from degrees into radians when you calculate the velocity component based on the angle.}. In the figure, the horizontal velocity component is designated \(u\).

Along with the schematic of the code structure, this should be all you need to create a basic code.\footnote{To recap once more:\\ You should have a constant defined, and then 5 \textit{parameters} --  3 representing the initial conditions of the model (the parts  Figure \ref{fig:ch9-ball} colored in red), plus 2 parameters for the maximum time and time step length. You also have 3 \textit{variables} in the code so far -- time \(t\), which is part of the loop, (horizontal) position \(p\), which you should have initialized to zero before the loop starts, and  (horizontal) velocity component \(u\), which you should have initialized calculated from \(s_{0}\) and \(\phi\). There should be nothing in the loop so far.} Check that it runs without error even though it is doing nothing useful yet! Maybe add some debug (e.g. a line in the loop using \texttt{disp}) to check that the loop really does loop from zero to \texttt{max\_t} in steps of \texttt{dt}.\sidenote{Note that depending on whether or not \texttt{max\_t} is divisible by \texttt{dt} with no remainder, your loop might not exactly finish at a value for \texttt{a} of \texttt{dt}.}
...
\\ \ \ \ \ \textcolor[rgb]{0,0.501961,0}{\% (YOUR CODE TO UPDATE POSITION GOES\ HERE)}
...
\newthought{\textit{\textbf{Part II }}} Now for tracking the vertical position (and velocity) of the ball. You can copy your previous \textsf{m-file} and use it as a starting point for the new model\sidenote{So for instance we will now interpret the variable \(p\) as the vertical, not horizontal position of the ball.} or start a fresh \textit{script} m-file -- your call.
...
In the loop add code to calculate the change in velocity during the time-step, and then update the value of \(v\)\sidenote{Hint:
\begin{docspecmargin}
\(v_{(t+1)}=v_{(t)}+\Delta v\)
\end{docspecmargin} where \(v_{(t+1)}\) is the new (at the next time-step) velocity and  \(v_{(t)}\) the current velocity}\sidenote{Note that in this example and as per Figure \ref{fig:ch9-schematic-ball_v}, we update the vertical position in the loop \uline{first} (at the start of the loop), and then update the velocity afterwards.}. (Pay attention to the margin notes and program schematic if you are stuck in either the equation, or where to put the code ...)

Re-run the model ... what happens? Does this seem at all 'reasonable'? At this point you might consider whether you really do need to run the model for as long as \(100s\). Play about with the assumed initial angle and also the velocity (assuming plausible values in each case \textgreater\textgreater\textgreater) and get a feel for what is the longest duration (in \(s\)) that the ball lasts in the air (i.e. until its position becomes negative) -- this value will guide your choice of a more practical maximum time limit in your loop definition (remembering that you defined the loop limit in time as a parameter near the start of your program). 
...
\newthought{\textit{\textbf{Part III }}} By this point, you should now have 2 working models (and 2 seperate \textbf{m-files}) -- one for the horizontal position of the ball, and one for the vertical position (and vertical velocity) of the ball. You now need to combine the 2 sperate parts of the model into a single (new) program (e.g. named \textsf{ball\_uv.m}).\sidenote{I suggest basing the combined model on the vertical model (as it is the more complicated of the 2) and hence copying-and-renaming the 2nd script (i.e. so you end up with 3 different \textbf{m-files} by the end of the exercise).}
...
By now, you should have noted a slight problem -- in both previous (sperate) models, the variable \(p\) was used to represent both the horizontal AND vertical position of the ball. \docenv{D'uh!}\marginnote{
\docenvdef{duh}
...
My solution would be ... a vector to store the current position -- just of one row and two columns, i.e. exactly as you might write a position in \((x,y)\) notation. The horizontal position (\(x\)) is hence assigned the first element (\texttt{p(1)}) and the vertical position, the 2nd (\texttt{p(2)}). If you do this, you'll need to edit how you set the initial conditions in the code, e.g.
\begin{docspecbold}
p(1) = 0;
\\p(2) = h;
\end{docspecbold}
...
\ \ \ p(2) = p(2) + dp;
\end{docspecbold}
just as long as you \uline{comment} your code and do not get the re-use of the same variable name \texttt{dp} mixed up.

Hopefully this works and runs ... Maybe add some output within the loop to track its progress, such as:
\begin{docspecmargin}
\textcolor{blue}{for} t = 0:dt:max\_t
\\ \ \ \ \ \textcolor[rgb]{0,0.501961,0}{\% (YOU CODE TO UPDATE POSITION)}
\\ \ \ \ \ disp([\textcolor[rgb]{1,0,1}{'('}, num2str(p(1)), \textcolor[rgb]{1,0,1}{','}, num2str(p(2)), ...
\\ \ \ \ \ \ \ \ \textcolor[rgb]{1,0,1}{') @ t = '}, num2str(t)]);
\\ \ \ \ \ \textcolor[rgb]{0,0.501961,0}{\% (YOUR CODE TO UPDATE THE 2 VELOCITY COMPONENTS)}
\\\textcolor{blue}{end}
\end{docspecmargin}
...
As a departure from previous plotting, rather than saving all the data (time and position) and then plotting   at the very end (after the loop)\sidenote{Although if you stored the position of the ball at each time-step, you could re-play the trajectory afterwards.}, are are going to plot each new position of the ball as it is calculated, all \uline{within} the loop.

Take your previous code (either your 3rd script -- \textsf{ball\_uv.m} or the variant where you substituted the \texttt{for} loop for a \texttt{while} loop) -- copy and rename the \textbf{m-file}, and then open a new graphics figure window, before the loop starts, and set \texttt{hold on}, by adding the lines
\begin{docspecbold}
figure;
\\hold on;
\end{docspecbold}

\uline{Within} the loop, you want to plot each \((x,y)\) position as it is calculated (i.e. \uline{after} the horizontal and vertical positions have been updated) by:
\begin{docspecbold}
\ \ \ \ scatter(p(1),p(2));
\end{docspecbold}
(feel free to add additional parameters to \texttt{scatter} to make the points smaller or larger, or filled, or whatever).

Well, not so exciting. The plots sort of appears all at once and there is no sense of animation or of the ball moving. \textbf{MATLAB} is just way too fast for its own good\sidenote{This is a Trump true fact. In truth, \textbf{MATLAB} is about the slowest piece of *\$\&\% about.}.

You can make the loop proceed slower, by adding a time delay -- i.e. each time around the loop, \textbf{MATLAB} will take whatever time it needs to carry out the calculation and plot the current position PLUS whatever additional time you tell it to chill out for. The \textbf{MATLAB} command (a \textit{function}) is called \docenv{pause} and you might initially try in your code:
\begin{docspecbold}
\ \ \ \ pause(0.05);
\end{docspecbold}
which should insert a \(50 ms\) delay into the loop. Add the pause either immediately before, or immediately after, \texttt{scatter} (it doesn't matter which). Run what you have so far.
...
Now it has all got really trippy. Clearly, if you tell it nothing else, \textbf{MATLAB} will insist on auto-scaling the (\(x\) and \(y\)) limits of the plot. As the position of the ball increases (initially) in \textit{y}-axis direction, and (constantly) along the \textit{x-axis} direction, \textbf{MATLAB} periodically re-scales the axes. Annoying. So before the loop starts and after you create the figure window, why not prescribe some axes limits(?)\footnote{Remember: the \textit{function} is \texttt{axis}}\footnote{And also add axis labels ...} Having played with the model previously, you should have a good feel for what the maximum vertical and horizontal distances are achieved associated with 'reasonable' choices for the initial conditions (\(s_{0}\) and \(\theta\)).

You should end up with something looping like Figure \ref{fig:ch9-ball_uv}-ish.
...
If, in your \textit{loop}, after updating the position of the ball, you write (in place or whatever \texttt{pause} and scatter \texttt{code} lines you had before):
\begin{docspecmargin}
\ \ \ pause(0.025);\\
\ \ \ delete(h);\\
...
\end{docspecmargin}
you should see a red ball (with a black outline) smoothly sailing across the screen (but see margin notes ...).\sidenote[][-1.0in]{You could make the animation a little smoother by decreasing the time-step and also playing about with the delay (\texttt{pause}).}
\sidenote[][-0.0in]{
...
\noindent [OPTIONAL] A further refinement would be to add a term to account for air resistance -- as the ball travels through the air, friction will act to decelerate the ball. 
...
Now you are going to want to think about the design of the \textit{GUI} a little. Ideally -- sketch out a design on paper\sidenote{The white flat stuff that you write on. Maybe you have forgotten what it is? Clue: it is not an app on iTunes.} first.
...
\end{docspecbold}\vspace{-1mm}
This is as simple(!) as asking to find the ID of the object which has a \textsf{Tag} with value '\textsf{answertextbox}' (which was the value I set in the design editor -- if you chose a different name, or did not change the default name, you will need to adjust this line of code to reflect that).\sidenote{What we might refer to as an ID, \textbf{MATLAB} calls a \textit{handle}. Hence commonly an '\texttt{h}' might appear at the start of a variable name to indicate it contains a \textit{handle}.} You could put this line of code at the start (after the \textit{function} definition, and after the initial comment lines) of the \textit{function} \texttt{yesbutton\_Callback} and because you have two buttons, both of which will need to be able to change the text in the 2nd text box -- you place the same line of code also in the \textit{function} \linebreak \texttt{nobutton\_Callback}

\vspace{4mm}
\pagebreak

Now that we have the ID of the 2nd text box (assigned to the variable \texttt{h\_answertext}), we can  set its property (from containing no text at all, to displaying a suitable message). 

Lets first implement an answer if the Yes push button is clicked. The command to set a property is ... \docenv{set}. In our example, we obtained the handle of the text box object and assigned it to the variable \texttt{h\_answertext}. The name of the property we want to change (refer to the column list in the property editor if you like as a reminder) is \texttt{'String'}.  And the text ... well, you can have whatever you want. The complete line, which goes after you have assigned the test box handle to \texttt{h\_answertext}, is:
\begin{docspecbold}
...
\texttt{set(h\_answertext,'Backgroundcolor','g');}} Try this -- i.e. obtain the handle of the Yes button (and e.g. assign to to the variable \texttt{h\_button}) within the \texttt{\textcolor{blue}{function} yesbutton\_Callback} \textit{function} code, and set a (different) color.
...
\end{docspec}
(which goes in the \texttt{slider1\_Callback} \textit{function}).
...
\end{docspec}
(here again ... not the greatest of \textit{variable} name choices ...)
...
There are various further refinements you could make, such as when the program/app starts up, you could read the default value of the \textsf{Slider} and update the display (the \textsf{Static Text} box)\sidenote{Code going in the \textit{function}, \texttt{OpeningFcn}.}. You might also add a \textsf{Push Button} to close the app.

\vspace{1mm}
\noindent\rule{4cm}{0.5pt}
\vspace{2mm}

\noindent As another test of your programming and use of \textit{conditionals} (e.g. \texttt{\textcolor{blue}{if}}s) ... if the value of \texttt{bananaindex} goes above 5, set the text box background to blue. And if the value of \texttt{bananaindex} is below (or equal to) 5, set the color to red.\sidenote{The \textsf{Static Text} box property is called \textsf{Foregroundcolor}. To set, e.g. add the code:
\begin{docspecmargin}
...
Because of the complexity of the project, the complete code (\textbf{m-file}) is provided on the course webpage \uline{if} needed for guidance (e.g. what code goes where?). Example images are also provided (download via the course webpage) but you can substitute your own if you prefer.
...
\newthought{\textit{\textbf{Part \(0\)}}} -- A few of the graphics procedures you will need to grasp and implement. \footnote{\textcolor{red}{\textbf{Example codes provided}}}

Firstly, at the command line, open a \textsf{Figure window} (\texttt{>> figure}).
Download any (legal/moral) image you care from the internet\sidenote{With the  raster graphics format being one of: \textsf{.jpg}, \textsf{.png}, \textsf{.tif}.}. You can load this image into the \textbf{MATLAB} workspace with \docenv{imread}, and display it in the \textsf{Figure window} with \docenv{imshow}. Try it. This fills up the screen, which is OK for a background image, but not for much else.  
...
>> imshow(A);
\end{docspecbold}
\sidenote{Or whatever you called the variable with the image in.} you should see a smaller version of the image, positioned at the origin. If you assign the handle returned by \texttt{imshow} to a variable, by typing:
\begin{docspecbold}
>> h = imshow(A);
\end{docspecbold}
...
\end{docspec}
For \texttt{scatter}-plotting, you then specified what (\textit{x,y}) limits the axes represented, e.g.:
\begin{docspec}
axis([0 x\_max 0 y\_max]);
\end{docspec}
(here, using parameters containing the maximum \textit{x} and \textit{y} limits in placed of fixed / hard-coded values).
\\Now, instead of \texttt{scatter}-plotting the position of the ball -- within the loop, you are defining a small axes region to contain the image. (And whose location and width/height are given in relative (\(0-1\) scale) units, rather than \(0-x_{max}\) and \(0-y_{max}\).)
...
One final thing now is the background to the Pok\'emon image. The original format (\textsf{png}) is  defined with a transparent background. \textbf{MATLAB} can make use of this with a small tweak to the code -- replacing the \texttt{img\_eevee = } line with:
\begin{docspec}
...
\end{docspec}
which implements the transparent background -- Figure \ref{fig:chGUI-Pokemon5}.
